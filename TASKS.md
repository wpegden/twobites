...
- [ ] Extend the huge-case bookkeeping in `repo/Twobites/IndependentSets.lean` from the new choose-of-weight and projection-union-plus-error bounds on `H_I \cap V_R` / `H_I \cap V_B` to the paper's capped-sum convexity and min-expression bounds for the cross-projection terms in Lemma `lem:huge`.
