...
-- [TABLET NODE: FixedSetHistoryCellMixedBranchTranscriptWeightPackage]

theorem FixedSetHistoryCellMixedBranchTranscriptWeightPackage :
    ∀ {n m uR uB : ℕ} {p a : ℝ}
...
-- BODY
  classical
  intro n m uR uB p a hist target terminal order blueSupportLabels
    BranchLabel _ blueTrace transcriptLabels RedLabel _ redSupport cellEvent
    hp_nonneg hp_half horder_nodup horder_terminal hproduct_law hblue_labels
    _hblueTrace_functional hcell_geometry cellRealized hrealized_geometry
  ...
  let fixedFactor : ℝ :=
    p ^ uR * p ^ uB *
      Real.rpow q (max 0 (a - (uR : ℝ) - (uB : ℝ)))
  let cellWeight :
      Finset Coord → RedLabel → BranchLabel → Transcript → ℝ :=
    fun B J β τ =>
      if B ∈ blueSupportLabels ∧ τ ∈ transcriptLabels ∧
  ...
  · intro B hB J
    ...
    let blueCoord : Coord → Prop :=
      fun e =>
        ...
        | Sum.inl _ => False
        | Sum.inr _ => True
    let redSupportJ : BranchLabel → Finset Coord :=
      fun β => redSupport β J
    ...
    have hgeom_pack :
        ∀ β τ,
          τ ∈ transcriptLabels →
            realizedJ β τ →
              (redSupportJ β).card = uR ∧
              redSupportJ β ⊆ terminal ∧
              Disjoint B (redSupportJ β) ∧
              present τ ⊆ terminal ∧
              absent τ ⊆ terminal ∧
              Disjoint (present τ) (absent τ) ∧
              B ∪ redSupportJ β ⊆ present τ ∧
              (∀ e,
                e ∈ terminal →
                  blueCoord e →
      ...
      have hBRdisj : Disjoint B (redSupport β J) := by
    ...
    obtain ⟨residualMass, hres_nonneg, hres_sum, hres_dom⟩ :=
      FixedSetHistoryCellFixedBJResidualProductTreePacking
        ...
        (blueTrace := blueTrace) (redSupport := redSupportJ)
