...
-- [TABLET NODE: ParameterHierarchyT8Threshold]

open Filter
open scoped Topology

theorem ParameterHierarchyT8Threshold :
    ∀ η : ℝ, 0 < η →
      ∃ n0 : ℕ, ∀ n : ℕ, n0 < n →
        let L := Real.log (n : ℝ)
...
-- BODY
  ...
  have hmain_eventually :
    ...
    have hleft_le_sqrtn :
        200 * (5 * κ) ^ 2 * ((Real.log L) ^ 2 * L ^ 3) ≤
          κ * Real.sqrt (n : ℝ) := by
      calc
        200 * (5 * κ) ^ 2 * ((Real.log L) ^ 2 * L ^ 3)
            ≤ 200 * (5 * κ) ^ 2 * (c ^ 5 * Real.sqrt (n : ℝ)) :=
          mul_le_mul_of_nonneg_left hpoly_le (by positivity)
        _ = (200 * (5 * κ) ^ 2 * c ^ 5) * Real.sqrt (n : ℝ) := by ring
        _ ≤ κ * Real.sqrt (n : ℝ) :=
    ...
    have hnum_le_den :
        200 * (5 * (1 + η)) ^ 2 * (Real.log L) ^ 2 * L ^ 3 ≤
          (1 + η) * Real.sqrt ((n : ℝ) * L) := by
      calc
        200 * (5 * (1 + η)) ^ 2 * (Real.log L) ^ 2 * L ^ 3
            = 200 * (5 * κ) ^ 2 * ((Real.log L) ^ 2 * L ^ 3) := by
          dsimp [κ]
          ring
        _ ≤ κ * Real.sqrt (n : ℝ) := hleft_le_sqrtn
        _ ≤ κ * Real.sqrt ((n : ℝ) * L) :=
          mul_le_mul_of_nonneg_left hsqrtn_le_sqrtnL (le_of_lt hκ_pos)
        _ = (1 + η) * Real.sqrt ((n : ℝ) * L) := by dsimp [κ]
    have hden_pos :
        0 < (1 + η) * Real.sqrt ((n : ℝ) * L) := by
