...
-- [TABLET NODE: FixedSetHistoryCellFixedBJResidualPartitionData]

theorem FixedSetHistoryCellFixedBJResidualPartitionData :
    ∀ {n m uR uB : ℕ} {p a : ℝ}
...
-- BODY
  ...
  intro n m uR uB p a hist terminal order B BranchLabel _
    blueTrace transcriptLabels RedLabel _ J redSupport cellRealized
    assignmentCompatible hp_nonneg hp_half _horder_nodup _horder_terminal
    _hproduct_law hBcard hBterminal hBblue _hblueTrace_functional
  ...
  · intro β τ hτ hreal
    let R : Finset Coord := redSupport β J
    ...
    have hBRdisj : Disjoint B R := by
      ...
      cases e with
      | inl e =>
          exact hBblue (Sum.inl e) heB
      | inr e =>
          exact hRred (Sum.inr e) heR
    ...
    have hresidualPresent_card :
        residualPresent.card = τ.1.card - uR - uB := by
