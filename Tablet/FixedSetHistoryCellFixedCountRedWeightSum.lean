...
-- BODY
  ...
  intro n m k ℓR ℓB p ε ε1 ε2 I hist recorded terminal order rep uR uB
      ...
      hterminal_universe horder_nodup horder_terminal hblue_before_red
  ...
  have hfixed_BJ_residual_certificate :
    ...
    have hBcard : B.card = uB := (hblue_labels B hB).1
    ...
    have hBblue :
        ∀ e,
          e ∈ B →
            match e with
            | Sum.inl _ => False
            | Sum.inr _ => True := (hblue_labels B hB).2.2
    ...
    have hscan_inputs :
      ...
      have hfirst_mismatch_unique :
          ...
          let fixed : Finset Coord := B ∪ redSupport β J
          let fixed' : Finset Coord := B ∪ redSupport β' J
          ...
          · let mismatch : Coord → Prop :=
            ...
            have hblue_trace_eq_if_red :
                ...
                have hcPre : c ∈ pre.toFinset :=
                  hblue_before_red pre e suffix hsplit heRed c hcTerminal hcBlue
                exact
            ...
            have hfixed_for_value :
              ...
              | inl q =>
                  ...
                  have heR : Sum.inl q ∈ redSupport β J := by
                    have heFilter :
                        Sum.inl q ∈
            ...
            have hcontr_left :
              ...
              · rcases Finset.mem_union.mp heFixed with heB | heR
                ...
                · have heRed : TwoBiteCoordinateIsRed e := by
                    cases hcase : e with
                    | inl q => trivial
                    | inr q =>
                        have heR_q : Sum.inr q ∈ redSupport β J := by
                          simpa [hcase] using heR
                        exact (hRred (Sum.inr q) heR_q).elim
                  have hβeq : β = β' := by
                    apply hblueTrace_functional
                    intro c hc hblue
                    exact hblue_trace_eq_if_red heRed c hc hblue
                  have heR' : e ∈ redSupport β' J := by
    ...
    have hresidual_inputs_grouped :=
      FixedSetHistoryCellFixedBJResidualCompatibilityFromScanData
        ...
        (blueColor := blueCoord) (blueTrace := blueTrace)
        ...
        hblueTrace_functional hgeometry_for_scan hcompat_forward_full_pack
          ...
          A ⊆ terminal →
            τ ∈ transcriptLabels →
              ...
              (τ.1 \ (B ∪ redSupport β J)) ⊆ A →
                Disjoint (τ.2.1 \ τ.2.2) A →
                  assignmentCompatible
                    ((A \ τ.2.2) ∪ B ∪ redSupport β J) B J β τ := by
      intro A β τ hA hτ hreal hpresentResidual habsentResidual
      have hcompat :=
          ...
          τ ∈ transcriptLabels →
            τ' ∈ transcriptLabels →
              cellRealized B J β τ →
                cellRealized B J β' τ' →
                  assignmentCompatible
                    ((A \ τ.2.2) ∪ B ∪ redSupport β J) B J β τ →
                    assignmentCompatible
                      ((A \ τ'.2.2) ∪ B ∪ redSupport β' J) B J β' τ' →
                      β = β' ∧ τ = τ' := by
      intro A β β' τ τ' hA hτ hτ' hreal hreal' hcompat hcompat'
    ...
    obtain
      ...
      FixedSetHistoryCellFixedBJResidualReleasedBlockData
        (n := n) (m := m) (uR := uR) (uB := uB) (p := p) (ε := ε)
        (a := a) I hist targetHist recorded terminal order B blueTrace
        transcriptLabels J redSupport cellEvent cellRealized
    ...
    obtain
        ⟨_residualOwner, _howner_cylinder, _howner_functional,
          ...
          hleaf_dom⟩ :=
      FixedSetHistoryCellFixedBJResidualProductTreeOwnerPackage
        ...
        hBcard hBterminal hBblue hblueTrace_functional hfixed_geometry
