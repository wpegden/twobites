...
-- [TABLET NODE: FixedSetHistoryCellRedFixedCountCellSummation]

theorem FixedSetHistoryCellRedFixedCountCellSummation :
    ∀ {n m uR uB NR NB : ℕ} {p a : ℝ}
      ...
      (redSupport :
        BranchLabel →
          RedLabel → Finset (Sum (Fin m × Fin m) (Fin m × Fin m)))
      (cellEvent :
        Finset (Sum (Fin m × Fin m) (Fin m × Fin m)) →
          RedLabel →
          BranchLabel →
...
-- BODY
  classical
  intro n m uR uB NR NB p a hist target terminal blueSupportLabels
    BranchLabel _ transcriptLabels RedLabel _ redSupport cellEvent cellWeight
  ...
  let PairIndex := {B : Finset Coord // B ∈ blueSupportLabels} × RedLabel
  let TranscriptIndex := {τ : Transcript // τ ∈ transcriptLabels}
  let prob : (Ω → Prop) → ℝ := fun E => TwoBiteEventProbability n m p E
  let L : ℝ := max 0 (a - (uR : ℝ) - (uB : ℝ))
  let pairMass : ℝ :=
    p ^ uR * p ^ uB * Real.rpow ((1 : ℝ) - p) L
  let rhs : ℝ :=
    (Nat.choose NR uR : ℝ) * p ^ uR *
      (Nat.choose NB uB : ℝ) * p ^ uB *
        Real.rpow ((1 : ℝ) - p) L
  ...
  have htranscript_sum :
      ∀ (B : Finset Coord) (J : RedLabel) (β : BranchLabel),
  ...
  have hpair_card :
      Fintype.card PairIndex =
        blueSupportLabels.card * Fintype.card RedLabel := by
    simp [PairIndex, Fintype.card_prod, Fintype.card_coe]
  have hpair_card_bound :
      (Fintype.card PairIndex : ℝ)
        ≤ (Nat.choose NB uB : ℝ) * (Nat.choose NR uR : ℝ) := by
    have hblue_card_real :
        (blueSupportLabels.card : ℝ) ≤ (Nat.choose NB uB : ℝ) := by
      exact_mod_cast hblue_card
    have hred_card_real :
        (Fintype.card RedLabel : ℝ) ≤ (Nat.choose NR uR : ℝ) := by
  ...
  have hweight_sum :
    ...
    calc
      ...
      _ =
          ...
          simp
      _ ≤
          ((Nat.choose NB uB : ℝ) * (Nat.choose NR uR : ℝ)) *
            (prob hist * pairMass) := by
