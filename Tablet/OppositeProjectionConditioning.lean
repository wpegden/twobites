...
-- [TABLET NODE: OppositeProjectionConditioning]

theorem OppositeProjectionConditioning (n m : ℕ) (p : ℝ)
    (event : TwoBiteSample n m p → Prop) (C : ℝ)
    (hC : 0 ≤ C)
    (hweight : ∀ ω : TwoBiteSample n m p, 0 ≤ TwoBiteSampleWeight ω)
    (bound : ∀ (G_R : SimpleGraph (Fin m)) (ρ : Fin n → Fin m),
      TwoBiteConditionalProbability n m p event (fun ω => ω.1 = G_R ∧ (fun v => (ω.2.2 v).1) = ρ) ≤ C) :
    TwoBiteEventProbability n m p event ≤ C := by
-- BODY
  classical
  let proj : TwoBiteSample n m p → SimpleGraph (Fin m) × (Fin n → Fin m) :=
    fun ω => (ω.1, fun v => (ω.2.2 v).1)
  ...
  have H_cell : ∀ val : SimpleGraph (Fin m) × (Fin n → Fin m),
    ...
    have heq : (fun ω : TwoBiteSample n m p => ω.1 = val.1 ∧ (fun v => (ω.2.2 v).1) = val.2) = 
               (fun ω => proj ω = val) := by
      ext ω
      unfold proj
  ...
  rw [H_event]
  calc
    ∑ val : SimpleGraph (Fin m) × (Fin n → Fin m), TwoBiteEventProbability n m p (fun ω => event ω ∧ proj ω = val)
      ≤ ∑ val : SimpleGraph (Fin m) × (Fin n → Fin m), C * TwoBiteEventProbability n m p (fun ω => proj ω = val) := by
        apply Finset.sum_le_sum
        intro val _
        exact H_cell val
    _ = C * ∑ val : SimpleGraph (Fin m) × (Fin n → Fin m), TwoBiteEventProbability n m p (fun ω => proj ω = val) := by
        rw [← Finset.mul_sum]
