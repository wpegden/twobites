...
import Tablet.FixedSetHistoryCellFixedCountSummationBridge
...
-- [TABLET NODE: FixedSetHistoryCellFixedCountProductBound]

theorem FixedSetHistoryCellFixedCountProductBound :
  ...
  · rcases hbridge.1 horder_counts with ⟨NR, NB, hNR, hNB, hprob⟩
    have hbinom :
        (Nat.choose NR uR : ℝ) * p ^ uR *
          ...
          Real.rpow ((1 : ℝ) - p) (max 0 (a - (uR : ℝ) - (uB : ℝ)))
          ≤ Real.exp (8 * Real.sqrt ε1 * p * (k : ℝ) ^ 2 - p * a) := by
      simpa [a] using
        FixedSetHistoryCellBinomialExponentialEstimate
          (k := k) (uR := uR) (uB := uB) (NR := NR) (NB := NB)
          (p := p) (ε1 := ε1) (a := a)
          hε1_nonneg hε1_le_one hp_nonneg hp_le_half horder_counts hNR hNB
    have hmain := le_trans (by simpa [a] using hprob) hbinom
    exact hmain
  · have horder_counts' : uR ≤ uB := Nat.le_of_not_ge horder_counts
    rcases hbridge.2 horder_counts' with ⟨NR, NB, hNR, hNB, hprob⟩
    have hbinom :
        (Nat.choose NR uB : ℝ) * p ^ uB *
            (Nat.choose NB uR : ℝ) * p ^ uR *
          Real.rpow ((1 : ℝ) - p) (max 0 (a - (uB : ℝ) - (uR : ℝ)))
          ≤ Real.exp (8 * Real.sqrt ε1 * p * (k : ℝ) ^ 2 - p * a) := by
      simpa [a] using
        FixedSetHistoryCellBinomialExponentialEstimate
          (k := k) (uR := uB) (uB := uR) (NR := NR) (NB := NB)
          (p := p) (ε1 := ε1) (a := a)
          hε1_nonneg hε1_le_one hp_nonneg hp_le_half horder_counts' hNR hNB
    have hmain := le_trans (by simpa [a] using hprob) hbinom
    exact hmain
