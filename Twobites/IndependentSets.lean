...
theorem imageSum_card_le_paperKNat_of_paperNearOneReal
    (C : ConstructionData n m) (I : Finset (Fin n)) {ε : ℝ}
    (hsum :
      ((((C.redImage I).card + (C.blueImage I).card : ℕ) : ℝ)) ≤
        (1 + ε / 2) * Twobites.paperK (1 + ε) n) :
    (C.redImage I).card + (C.blueImage I).card ≤
      Twobites.paperKNat (((1 + ε) * (2 + ε)) / 2) n := by
  have hcast :
      ((((C.redImage I).card + (C.blueImage I).card : ℕ) : ℝ)) ≤
        Twobites.paperK (((1 + ε) * (2 + ε)) / 2) n := by
    calc
      ((((C.redImage I).card + (C.blueImage I).card : ℕ) : ℝ)) ≤
          (1 + ε / 2) * Twobites.paperK (1 + ε) n := hsum
      _ = Twobites.paperK (((1 + ε) * (2 + ε)) / 2) n := by
          rw [Twobites.mul_paperK_eq_paperK_mul]
          congr 1
          ring
  exact Twobites.le_paperKNat_of_cast_le_paperK hcast

theorem blueImage_card_le_paperKNat_of_paperNearOneReal_of_blue_le_red
    (C : ConstructionData n m) (I : Finset (Fin n)) {ε : ℝ}
    (hsum :
      ((((C.redImage I).card + (C.blueImage I).card : ℕ) : ℝ)) ≤
        (1 + ε / 2) * Twobites.paperK (1 + ε) n)
  ...
  have hsumNat :
      (C.redImage I).card + (C.blueImage I).card ≤
        Twobites.paperKNat (((1 + ε) * (2 + ε)) / 2) n :=
    C.imageSum_card_le_paperKNat_of_paperNearOneReal I hsum
  exact C.blueImage_card_le_paperKNat_of_paperNearOneSum_of_blue_le_red I hsumNat hblue hε

theorem redImage_card_le_paperKNat_of_paperNearOneReal_of_red_le_blue
    (C : ConstructionData n m) (I : Finset (Fin n)) {ε : ℝ}
    (hsum :
      ((((C.redImage I).card + (C.blueImage I).card : ℕ) : ℝ)) ≤
        (1 + ε / 2) * Twobites.paperK (1 + ε) n)
  ...
  have hsumNat :
      (C.redImage I).card + (C.blueImage I).card ≤
        Twobites.paperKNat (((1 + ε) * (2 + ε)) / 2) n :=
    C.imageSum_card_le_paperKNat_of_paperNearOneReal I hsum
  exact C.redImage_card_le_paperKNat_of_paperNearOneSum_of_red_le_blue I hsumNat hred hε
...
theorem
  ...
  have hblue :
      (C.blueImage I).card ≤ Twobites.paperKNat (((1 + ε) * (2 + ε)) / 4) n :=
    C.blueImage_card_le_paperKNat_of_paperNearOneReal_of_blue_le_red I hsum hblueLeRed hε0.le
  exact
    C.paper_risi_hLossGap_of_blueLeft_of_redRight_gapLower_of_paperNearOne_of_two_div_le_loglog
      I hI hblue hn hε0 hε1 hloglog hLoss
...
set_option linter.style.longLine false in
theorem
    paper_risi_hLossGap_of_blueRight_of_redLeft_gapLower_of_paperNearOneReal_of_red_le_blue_of_two_div_le_loglog
    (C : ConstructionData n m) (I : Finset (Fin n)) {ε ε1 : ℝ}
    (hI : I.card ≤ Twobites.paperKNat (1 + ε) n)
    (hsum :
      ((((C.redImage I).card + (C.blueImage I).card : ℕ) : ℝ)) ≤
  ...
  have hred :
      (C.redImage I).card ≤ Twobites.paperKNat (((1 + ε) * (2 + ε)) / 4) n :=
    C.redImage_card_le_paperKNat_of_paperNearOneReal_of_red_le_blue I hsum hredLeBlue hε0.le
  exact
    C.paper_risi_hLossGap_of_blueRight_of_redLeft_gapLower_of_paperNearOne_of_two_div_le_loglog
      I hI hred hn hε0 hε1 hloglog hLoss
...
theorem section4ActualConditionedEventMass_nonneg
    (C : ConstructionData n m) (I : Finset (Fin n)) {ε p : ℝ} {N : ℕ}
    (hp0 : 0 ≤ p) (hp1 : p ≤ 1) :
    0 ≤ C.section4ActualConditionedEventMass I ε p N := by
  let uRActual := (C.section4URedCondPairSet I (C.section4F I ε)).card
  let uBActual := (C.section4UBlueCondPairSet I (C.section4F I ε)).card
  let loss := C.section4SecondStageLossNat I ε
  have huR : (C.section4URedCondPairSet I (C.section4F I ε)).card = uRActual := rfl
  have huB : (C.section4UBlueCondPairSet I (C.section4F I ε)).card = uBActual := rfl
  calc
    0 ≤ section4BernoulliMass p uRActual uBActual (N - loss) := by
      exact section4BernoulliMass_nonneg hp0 hp1
    _ = C.section4ActualConditionedEventMass I ε p N := by
      unfold section4ActualConditionedEventMass
      simp [uRActual, uBActual, loss,
        C.section4UCondChoiceEventMass_eq_bernoulliMass_of_card_eq
...
theorem mul_section4ActualConditionedEventMass_le_exp_add
    ...
    (hinner : C.section4ActualConditionedEventMass I ε p N ≤ Real.exp innerExp) :
    outerMass * C.section4ActualConditionedEventMass I ε p N ≤
      Real.exp (outerExp + innerExp) := by
  have hinner0 : 0 ≤ C.section4ActualConditionedEventMass I ε p N :=
    C.section4ActualConditionedEventMass_nonneg I hp0 hp1
  have hmul :
      outerMass * C.section4ActualConditionedEventMass I ε p N ≤
        Real.exp outerExp * Real.exp innerExp := by
    exact mul_le_mul houter hinner hinner0 (by positivity)
...
set_option linter.style.longLine false in
/-- Paper inequality `eq:long`, expressed in the repo's current finite-mass language: an outer
exponential bound for the `\mathcal S_{I,\ell_R,\ell_B}` layer combines with the checked
Section 4 `lem:RISI` bound by adding exponents. -/
theorem paper_ri_eqLong_bound_of_outer_le_exp_of_paperRISILossGap
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε p ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB outerMass outerExp : ℝ}
    {mediumWitness smallBound : ℕ}
    (hindep :
      ∀ {v w : Fin n}, v ∈ I → w ∈ I → v ≠ w → ¬ C.finalGraph.Adj v w)
    (hp0 : 0 ≤ p) (hp1 : p ≤ 1)
      ...
      Real.exp
        (outerExp +
          (p * (12 * (ε1 * Twobites.paperK κ n ^ 2) +
            ...
            p * C.paperSection4OpenPairTarget I κ (Twobites.paperCapNat β ε2 n))) := by
  exact
    C.mul_section4ActualConditionedEventMass_le_exp_add I hp0 hp1 houter <|
      C.paper_risi_finiteMassBound_of_paperRISILossGap hD I hindep hp0 hp1 hHsubset ht21 ht32
        hn hε hI hκ hT2 hT1 hLChoose hLargeBound hMediumWitness hMediumBound hSmallCard
        hSmallBound hHChoose hRevealArith hLarge hMedium hSmall hHugeRed hHugeBlue hred
        hblue hblueCap hblueCapWeight hredCap hredCapWeight hρR hρB hβ hε2 hε1pos hε1le
        hloglogGap hdiagScale hcodegScale hsumGap hdegBound hchooseCodegBound hcodegBound
...
/-- Specialize the outer-event exponential interface for Paper Lemma `lem:RI` to the exact
family `\mathcal S_{I,\ell_R,\ell_B}` at the actual image sizes of `I`. -/
theorem paper_ri_outerEventMass_le_exp_of_paperParameterLogs_of_images
    (C : ConstructionData n m) (I : Finset (Fin n))
    {ε xR xB outerExp : ℝ}
    (hn : 1 < n)
    ...
    (hsum2 : xR + xB ≤ 2)
    (hk : Twobites.paperKNat (1 + ε) n ≤ Twobites.paperMNat n * Twobites.paperMNat n)
    ...
    (houterExp :
      ((((xR + xB - 2) * (Twobites.paperKNat (1 + ε) n : ℝ)) *
        ...
        ((1 - xR) * (Twobites.paperKNat (1 + ε) n : ℝ)) * Real.log xR +
        ((1 - xB) * (Twobites.paperKNat (1 + ε) n : ℝ)) * Real.log xB +
        (1 + xR + xB) * (Twobites.paperKNat (1 + ε) n : ℝ)) ≤
        outerExp) :
    Twobites.paperRIOuterEventMass (Twobites.paperMNat n) (C.redImage I).card
        (C.blueImage I).card (Twobites.paperKNat (1 + ε) n) ≤
      Real.exp outerExp := by
  have hbase :=
    Twobites.paperRIOuterEventMass_le_exp_of_paperParameterLogs
      (n := n) (ε := ε) (xR := xR) (xB := xB)
      hn hloglog hε hkone hsum2 hk hredM hblueM hkprod hhalf hredEq hblueEq hxRpos hxBpos
  exact hbase.trans (Real.exp_le_exp.mpr houterExp)

/-- Package the exact outer event family `\mathcal S_{I,\ell_R,\ell_B}` into the abstract
`eq:long` bridge by using the actual image sizes of `I` and the paper-parameter outer logarithmic
bound. -/
theorem paper_ri_eqLong_bound_of_outerEventMass_le_exp_of_paperParameterLogs_of_le
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β ε p ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB totalExp outerExp xR xB : ℝ}
    {mediumWitness smallBound : ℕ}
    (hindep :
      ∀ {v w : Fin n}, v ∈ I → w ∈ I → v ≠ w → ¬ C.finalGraph.Adj v w)
    (hp0 : 0 ≤ p) (hp1 : p ≤ 1)
    ...
    (ht21 : Twobites.paperT2 ε n ≤ Twobites.paperT1 n)
    (ht32 : Twobites.paperT3 ε n ≤ Twobites.paperT2 ε n)
    (hn : 1 < n) (hεnonneg : 0 ≤ ε) (hεquarter : ε ≤ (1 / 4 : ℝ))
    (hI : I.card ≤ Twobites.paperKNat (1 + ε) n)
    ...
    (htotal :
      outerExp +
          (p * (12 * (ε1 * Twobites.paperK (1 + ε) n ^ 2) +
              (⌈10 * (ε1 * Twobites.paperK (1 + ε) n ^ 2)⌉₊ : ℝ)) -
            p * C.paperSection4OpenPairTarget I (1 + ε) (Twobites.paperCapNat β ε2 n)) ≤
        totalExp) :
    Twobites.paperRIOuterEventMass (Twobites.paperMNat n) (C.redImage I).card
        (C.blueImage I).card (Twobites.paperKNat (1 + ε) n) *
        C.section4ActualConditionedEventMass I ε p (C.baseOpenPairSet I).card ≤
  ...
  have houter :=
    C.paper_ri_outerEventMass_le_exp_of_paperParameterLogs_of_images I hn houterLoglog hεnonneg
      hkone hsum2 hkOuter hredM hblueM hkprod hhalf hredEq hblueEq hxRpos hxBpos houterExp
  exact
    C.paper_ri_eqLong_bound_of_outer_le_exp_of_paperRISILossGap_of_le
      (κ := 1 + ε) hD I hindep hp0 hp1 hHsubset ht21 ht32 hn hεquarter hI hκ hT2 hT1
      hLChoose hLargeBound hMediumWitness hMediumBound hSmallCard hSmallBound hHChoose
      hRevealArith hLarge hMedium hSmall hHugeRed hHugeBlue hred hblue hblueCap
      hblueCapWeight hredCap hredCapWeight hρR hρB hβ hε2 hε1pos hε1le hloglogGap
      hdiagScale hcodegScale hsumGap hdegBound hchooseCodegBound hcodegBound hgap2R hκ2R
      hblueCrossSmall hgap2B hκ2B hredCrossSmall houter hLossGap htotal

/-- Specialize the union-factor outer-event exponential interface for Paper Lemma `lem:RI` to the
exact family `\mathcal S_{I,\ell_R,\ell_B}` at the actual image sizes of `I`. -/
theorem paper_ri_chooseOuterEventMass_le_exp_of_paperParameterLogs_of_images
    (C : ConstructionData n m) (I : Finset (Fin n))
    {ε xR xB outerExp : ℝ}
    (hn : 1 < n)
    (hloglog : 2 ≤ Real.log (Real.log (n : ℝ)))
    ...
    ((n.choose (Twobites.paperKNat (1 + ε) n) : ℕ) : ℝ) *
      ...
      Real.exp outerExp := by
  have hbase :=
    Twobites.paperRIChooseOuterEventMass_le_exp_of_paperParameterLogs
      (ε := ε) (xR := xR) (xB := xB) (n := n)
      (lR := (C.redImage I).card) (lB := (C.blueImage I).card)
      hn hloglog hε hεquarter hkone hsum2 hk hredM hblueM hkprod hhalf hredEq hblueEq
      hxRpos hxBpos
  exact hbase.trans (Real.exp_le_exp.mpr houterExp)

theorem redImage_scale_le_one_of_card_eq
    (C : ConstructionData n m) (I : Finset (Fin n))
    {ε xR : ℝ}
    (hkone : 1 ≤ Twobites.paperK (1 + ε) n)
    (hI : I.card ≤ Twobites.paperKNat (1 + ε) n)
    (hredEq : ((C.redImage I).card : ℝ) = xR * (Twobites.paperKNat (1 + ε) n : ℝ)) :
    xR ≤ 1 := by
  have hcard : (C.redImage I).card ≤ Twobites.paperKNat (1 + ε) n :=
    (C.redImage_card_le_card I).trans hI
  have hkpos : 0 < (Twobites.paperKNat (1 + ε) n : ℝ) := by
    have hnat : 1 ≤ Twobites.paperKNat (1 + ε) n := by
      exact Twobites.le_paperKNat_of_cast_le_paperK (by simpa using hkone)
    exact_mod_cast (lt_of_lt_of_le Nat.zero_lt_one hnat)
  have hcardR : ((C.redImage I).card : ℝ) ≤ (Twobites.paperKNat (1 + ε) n : ℝ) := by
    exact_mod_cast hcard
  rw [hredEq] at hcardR
  nlinarith

theorem blueImage_scale_le_one_of_card_eq
    (C : ConstructionData n m) (I : Finset (Fin n))
    {ε xB : ℝ}
    (hkone : 1 ≤ Twobites.paperK (1 + ε) n)
    (hI : I.card ≤ Twobites.paperKNat (1 + ε) n)
    (hblueEq : ((C.blueImage I).card : ℝ) = xB * (Twobites.paperKNat (1 + ε) n : ℝ)) :
    xB ≤ 1 := by
  have hcard : (C.blueImage I).card ≤ Twobites.paperKNat (1 + ε) n :=
    (C.blueImage_card_le_card I).trans hI
  have hkpos : 0 < (Twobites.paperKNat (1 + ε) n : ℝ) := by
    have hnat : 1 ≤ Twobites.paperKNat (1 + ε) n := by
      exact Twobites.le_paperKNat_of_cast_le_paperK (by simpa using hkone)
    exact_mod_cast (lt_of_lt_of_le Nat.zero_lt_one hnat)
  have hcardR : ((C.blueImage I).card : ℝ) ≤ (Twobites.paperKNat (1 + ε) n : ℝ) := by
    exact_mod_cast hcard
  rw [hblueEq] at hcardR
  nlinarith

/-- A cleaner version of the previous outer-event wrapper where the explicit exponent has already
been rewritten into the main `((x_R+x_B-1)/2) k log n` term plus the remaining lower-order
corrections, and the nonpositive `x \log x` pieces are dropped using the image-card scales
`x_R,x_B ≤ 1`. -/
theorem paper_ri_chooseOuterEventMass_le_exp_of_mainRemainder_of_images
    (C : ConstructionData n m) (I : Finset (Fin n))
    {ε xR xB outerExp : ℝ}
    (hn : 1 < n)
    (hloglog : 2 ≤ Real.log (Real.log (n : ℝ)))
    ...
    ((n.choose (Twobites.paperKNat (1 + ε) n) : ℕ) : ℝ) *
        Twobites.paperRIOuterEventMass (Twobites.paperMNat n) (C.redImage I).card
          (C.blueImage I).card (Twobites.paperKNat (1 + ε) n) ≤
      Real.exp outerExp := by
  have hxRle : xR ≤ 1 := C.redImage_scale_le_one_of_card_eq I hkone hI hredEq
  have hxBle : xB ≤ 1 := C.blueImage_scale_le_one_of_card_eq I hkone hI hblueEq
  have hmain :
      (((Twobites.paperKNat (1 + ε) n : ℝ) *
            (1 - Real.log (1 + ε) +
              (Real.log (n : ℝ) - Real.log (Real.log (n : ℝ))) / 2)) +
          ((((xR + xB - 2) * (Twobites.paperKNat (1 + ε) n : ℝ)) *
            ...
            (1 + xR + xB) * (Twobites.paperKNat (1 + ε) n : ℝ))) ≤
        outerExp := by
    exact (Twobites.paperRI_chooseOuterExp_le_mainRemainder hxRpos hxRle hxBpos hxBle).trans
      houterExp
  exact
    C.paper_ri_chooseOuterEventMass_le_exp_of_paperParameterLogs_of_images I hn hloglog hε
      hεquarter hkone hsum2 hk hredM hblueM hkprod hhalf hredEq hblueEq hxRpos hxBpos
      hmain

/-- The previous simplified outer-event exponent fed through the abstract `eq:long` bridge. This
reduces the remaining RI work to comparing the cleaner main-plus-remainder exponent against the
three manuscript regime coefficients. -/
theorem paper_ri_eqLong_bound_of_chooseOuterEventMass_le_exp_of_mainRemainder_of_le
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β ε p ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB totalExp outerExp xR xB : ℝ}
  ...
  have houter :=
    C.paper_ri_chooseOuterEventMass_le_exp_of_mainRemainder_of_images I hn houterLoglog
      hεnonneg hεquarter hI hkone hsum2 hkOuter hredM hblueM hkprod hhalf hredEq hblueEq
      hxRpos hxBpos houterExp
  have hκ : 1 ≤ 1 + ε := by linarith
  exact
    C.paper_ri_eqLong_bound_of_outer_le_exp_of_paperRISILossGap_of_le
      (κ := 1 + ε) (outerMass := ((n.choose (Twobites.paperKNat (1 + ε) n) : ℕ) : ℝ) *
        Twobites.paperRIOuterEventMass (Twobites.paperMNat n) (C.redImage I).card
          (C.blueImage I).card (Twobites.paperKNat (1 + ε) n))
      hD I hindep hp0 hp1 hHsubset ht21 ht32 hn hεquarter hI hκ hT2 hT1 hLChoose
...
/-- Package the union-factor exact outer event family `\mathcal S_{I,\ell_R,\ell_B}` into the
abstract `eq:long` bridge by using the actual image sizes of `I` and the paper-parameter outer
logarithmic bound. -/
theorem paper_ri_eqLong_bound_of_chooseOuterEventMass_le_exp_of_paperParameterLogs_of_le
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β ε p ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB totalExp outerExp xR xB : ℝ}
  ...
  have hκ : 1 ≤ 1 + ε := by linarith
  have houter :=
    C.paper_ri_chooseOuterEventMass_le_exp_of_paperParameterLogs_of_images I hn houterLoglog
      hεnonneg hεquarter hkone hsum2 hkOuter hredM hblueM hkprod hhalf hredEq hblueEq
      hxRpos hxBpos houterExp
  exact
    C.paper_ri_eqLong_bound_of_outer_le_exp_of_paperRISILossGap_of_le
      (κ := 1 + ε) (outerMass := ((n.choose (Twobites.paperKNat (1 + ε) n) : ℕ) : ℝ) *
...
-- The statement repeats the full cleaned `eq:long` exponent and exceeds the default
-- elaboration budget during definitional equality checks.
set_option linter.style.longLine false in
theorem paper_ri_eqLong_bound_largeSum_of_chooseOuterEventMass_le_exp_of_mainRemainder_of_images_of_le
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β ε p ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB outerExp xR xB : ℝ}
    {mediumWitness smallBound : ℕ}
    (hindep :
      ∀ {v w : Fin n}, v ∈ I → w ∈ I → v ≠ w → ¬ C.finalGraph.Adj v w)
    (hp0 : 0 ≤ p) (hp1 : p ≤ 1)
  ...
  have hmass :=
    C.paper_ri_eqLong_bound_of_chooseOuterEventMass_le_exp_of_mainRemainder_of_le hD I hindep
      hp0 hp1 hHsubset ht21 ht32 hn hεnonneg hεquarter hI hT2 hT1 hLChoose hLargeBound
      ...
      hκ2B hredCrossSmall houterLoglog hkone hsum2 hkOuter hredM hblueM hkprod hhalf
      hredEq hblueEq hxRpos hxBpos houterExp hLossGap htotal
  ...
  have hcoeff :
    ...
    have hcoeff0 :
      ...
      calc
        ...
        _ ≤ -(ε ^ 2 * (1 - 2 * ε - 2 * ε ^ 2)) / 4 :=
          Twobites.paperRI_largeSumCoeff_le_final hεnonneg hsum
    have hmulK :
        ((xR + xB - 1) / 2 -
              ((1 / 2 : ℝ) * (-2 * (1 + ε) + 2 * (1 + ε) * (xR + xB) -
                    2 * ε ^ 3 * (1 + ε)) / 2)) *
...
set_option maxHeartbeats 3000000 in
-- The statement repeats the full cleaned near-one `eq:long` exponent and exceeds the default
-- elaboration budget during definitional equality checks.
set_option linter.style.longLine false in
theorem paper_ri_eqLong_bound_nearOne_of_chooseOuterEventMass_le_exp_of_mainRemainder_of_images_of_le
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β ε p ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB outerExp xR xB : ℝ}
        ...
        -(1 - xR - xB) / 2 -
          ...
          (1 / (8 * (1 + ε))) *
            (2 * xB - 1 - (4 * ε ^ 2 + 2 * ε) * (xR + xB - 1) - 2 * ε * xR +
              4 * ε ^ 3 * (1 + ε) ^ 2) :=
            Twobites.paperRI_nearOne_mixedCoeff_eq hεnonneg
        _ ≤ ε * (-1 + ε + 22 * ε ^ 2) / (16 * (1 + ε)) :=
          Twobites.paperRI_nearOne_mixedCoeff_le_final hεnonneg hεsmall hsumLower hsumUpper
            hblueLe
    have hmulK :
        (-(1 - xR - xB) / 2 -
              (1 / (4 * (1 + ε))) *
...
-- large-sum Section 4 slack term `- 2 * δ * k * log n`.
set_option linter.style.longLine false in
theorem paper_ri_eqLong_bound_largeSum_of_chooseOuterEventMass_le_exp_of_mainRemainder_of_images_of_section_le
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β ε p ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB δ xR xB : ℝ}
    {mediumWitness smallBound : ℕ}
    (hindep :
      ∀ {v w : Fin n}, v ∈ I → w ∈ I → v ≠ w → ¬ C.finalGraph.Adj v w)
    (hp0 : 0 ≤ p) (hp1 : p ≤ 1)
      ...
      ((n.choose (Twobites.paperKNat (1 + ε) n) : ℕ) : ℝ) *
        ...
        Real.exp
          ((((xR + xB - 1) / 2) * (Twobites.paperKNat (1 + ε) n : ℝ) * Real.log (n : ℝ)) +
            ...
            (3 * (2 - xR - xB) * (Twobites.paperKNat (1 + ε) n : ℝ) * Real.log (2 : ℝ)) +
            ((1 - xR - xB) * (Twobites.paperKNat (1 + ε) n : ℝ) * Real.log (1 + ε)) +
            (2 + xR + xB) * (Twobites.paperKNat (1 + ε) n : ℝ)) := by
    exact
      C.paper_ri_chooseOuterEventMass_le_exp_of_mainRemainder_of_images I hn houterLoglog
        hεnonneg hεquarter hI hkone hsum2 hkOuter hredM hblueM hkprod hhalf hredEq hblueEq
        hxRpos hxBpos le_rfl
  have hinner :=
    C.paper_risi_finiteMassBound_of_paperRISILossGap hD I hindep hp0 hp1 hHsubset ht21 ht32
      hn hεquarter hI (by linarith : 1 ≤ 1 + ε) hT2 hT1 hLChoose hLargeBound hMediumWitness
      hMediumBound hSmallCard hSmallBound hHChoose hRevealArith hLarge hMedium hSmall
      hHugeRed hHugeBlue hred hblue hblueCap hblueCapWeight hredCap hredCapWeight hρR hρB
      hβ hε2 hε1pos hε1le hloglogGap hdiagScale hcodegScale hsumGap hdegBound
      hchooseCodegBound hcodegBound hgap2R hκ2R hblueCrossSmall hgap2B hκ2B hredCrossSmall
      hLossGap
  have hmass0 :
      ((n.choose (Twobites.paperKNat (1 + ε) n) : ℕ) : ℝ) *
          Twobites.paperRIOuterEventMass (Twobites.paperMNat n) (C.redImage I).card
        ...
        Real.exp
          (((((xR + xB - 1) / 2) * (Twobites.paperKNat (1 + ε) n : ℝ) * Real.log (n : ℝ)) +
            ...
            (p * (12 * (ε1 * Twobites.paperK (1 + ε) n ^ 2) +
                (⌈10 * (ε1 * Twobites.paperK (1 + ε) n ^ 2)⌉₊ : ℝ)) -
              p * C.paperSection4OpenPairTarget I (1 + ε) (Twobites.paperCapNat β ε2 n))) := by
    simpa [add_comm, add_left_comm, add_assoc] using
      C.mul_section4ActualConditionedEventMass_le_exp_add I hp0 hp1 houter hinner
  have htotal :
      ((((xR + xB - 1) / 2) * (Twobites.paperKNat (1 + ε) n : ℝ) * Real.log (n : ℝ)) +
      ...
      (((xR + xB - 1) / 2 -
          ...
          Twobites.paperK (1 + ε) n * Real.log (n : ℝ)) := by
    exact
      Twobites.paperRI_chooseOuterExp_add_risiLargeSum_le_htotal hn houterLoglog hε hεquarter
        hδ hδle hlargeLoglog hkone hsum hsum2 hsection
  have hmass :
      ((n.choose (Twobites.paperKNat (1 + ε) n) : ℕ) : ℝ) *
          Twobites.paperRIOuterEventMass (Twobites.paperMNat n) (C.redImage I).card
            (C.blueImage I).card (Twobites.paperKNat (1 + ε) n) *
        ...
        Real.exp
          ((((xR + xB - 1) / 2 -
                ((1 / 2 : ℝ) * (-2 * (1 + ε) + 2 * (1 + ε) * (xR + xB) -
                      2 * ε ^ 3 * (1 + ε)) / 2)) *
              Twobites.paperK (1 + ε) n * Real.log (n : ℝ))) := by
    exact hmass0.trans (Real.exp_le_exp.mpr htotal)
...
set_option linter.style.longLine false in
theorem paper_ri_eqLong_bound_nearOne_of_chooseOuterEventMass_le_exp_of_mainRemainder_of_images_of_section_le
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β ε p ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB δ xR xB : ℝ}
  ...
  have htotal :
      ...
      ((-(1 - xR - xB) / 2 -
          ...
          Twobites.paperK (1 + ε) n * Real.log (n : ℝ)) := by
    exact
      Twobites.paperRI_chooseOuterExp_add_risiNearOne_le_htotal hn houterLoglog hε hεquarter
        hδ hδle hlargeLoglog hkone hsumLower hsumUpper hsection
  have hmass :
      ((n.choose (Twobites.paperKNat (1 + ε) n) : ℕ) : ℝ) *
          Twobites.paperRIOuterEventMass (Twobites.paperMNat n) (C.redImage I).card
            (C.blueImage I).card (Twobites.paperKNat (1 + ε) n) *
        ...
        Real.exp
          (((-(1 - xR - xB) / 2 -
              ...
              Twobites.paperK (1 + ε) n * Real.log (n : ℝ))) := by
    exact hmass0.trans (Real.exp_le_exp.mpr htotal)
  exact C.paper_ri_eqLong_bound_nearOne_of_le I hn hεnonneg hεsmall hsumLower hsumUpper
    hblueLe hmass

set_option maxHeartbeats 3000000 in
-- This specialization discharges the near-one Section 4 slack term using the exact
-- `paperP(1/2)` mixed-branch budget from `ParameterBounds.lean`, assuming the
-- manuscript-scale blue-side cap gap is available as a natural inequality.
set_option linter.style.longLine false in
theorem
    paper_ri_eqLong_bound_nearOne_of_chooseOuterEventMass_le_exp_of_mainRemainder_of_images_of_budget_of_blueCapNat
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
  ...
  have hredK : (C.redImage I).card ≤ Twobites.paperKNat (1 + ε) n :=
    (C.redImage_card_le_card I).trans hI
  have hblueK : (C.blueImage I).card ≤ Twobites.paperKNat (1 + ε) n := by
    exact le_trans (Nat.le_add_right _ _) hblueCapNat
  have hdeltaLog : 4 / δ ≤ Real.log (Real.log (n : ℝ)) := by
    have hsmall : (4 : ℝ) / δ ≤ 72 / δ := by
      exact div_le_div_of_nonneg_right (by norm_num) hδ.le
    exact hsmall.trans hlargeLoglog
  have hblueCapCast :
      ((Twobites.paperKNat (1 + ε) n - (C.blueImage I).card -
            Twobites.paperCapNat (1 / 2) 0 n : ℕ) : ℝ) =
        Twobites.paperKNat (1 + ε) n - (C.blueImage I).card -
          Twobites.paperCapNat (1 / 2) 0 n := by
    have hsub :
        Twobites.paperKNat (1 + ε) n - (C.blueImage I).card -
            Twobites.paperCapNat (1 / 2) 0 n =
          Twobites.paperKNat (1 + ε) n -
            ((C.blueImage I).card + Twobites.paperCapNat (1 / 2) 0 n) := by
      omega
    rw [hsub, Nat.cast_sub hblueCapNat, Nat.cast_add]
    ring
  have htargetLower :
      (((xR + xB - 1) * (Twobites.paperKNat (1 + ε) n : ℝ)) *
            ((Twobites.paperKNat (1 + ε) n : ℝ) - 1) +
          (Twobites.paperCapNat (1 / 2) 0 n : ℝ) *
            ((1 - xB) * (Twobites.paperKNat (1 + ε) n : ℝ) -
              (Twobites.paperCapNat (1 / 2) 0 n : ℝ))) ≤
        C.paperSection4OpenPairTarget I (1 + ε) (Twobites.paperCapNat (1 / 2) 0 n) := by
    calc
      (((xR + xB - 1) * (Twobites.paperKNat (1 + ε) n : ℝ)) *
            ((Twobites.paperKNat (1 + ε) n : ℝ) - 1) +
          (Twobites.paperCapNat (1 / 2) 0 n : ℝ) *
        ...
        ((((C.redImage I).card + (C.blueImage I).card : ℕ) : ℝ) -
          ...
          (Twobites.paperCapNat (1 / 2) 0 n : ℝ) *
            ((Twobites.paperKNat (1 + ε) n - (C.blueImage I).card -
                  Twobites.paperCapNat (1 / 2) 0 n : ℕ) : ℝ) := by
        rw [Nat.cast_add, hblueCapCast, hredEq, hblueEq]
        ring_nf
      _ = ((C.redImage I).card.choose 2 : ℝ) + ((C.blueImage I).card.choose 2 : ℝ) -
            (((Twobites.paperKNat (1 + ε) n - (C.redImage I).card).choose 2 : ℕ) : ℝ) -
            C.paperHugeRedCrossRightTarget I (1 + ε) (Twobites.paperCapNat (1 / 2) 0 n) := by
        exact C.paperSection4Branch_blueLeft_redRight_eq I hredK hblueCapNat
      _ ≤ C.paperSection4OpenPairTarget I (1 + ε) (Twobites.paperCapNat (1 / 2) 0 n) :=
        C.paperSection4OpenPairTarget_lower_bound_of_blueLeft_of_redRight I
  have hsection0 :
      (Twobites.paperP (1 / 2) n *
            (12 * (ε1 * Twobites.paperK (1 + ε) n ^ 2) +
              (⌈10 * (ε1 * Twobites.paperK (1 + ε) n ^ 2)⌉₊ : ℝ)) -
          Twobites.paperP (1 / 2) n *
            C.paperSection4OpenPairTarget I (1 + ε) (Twobites.paperCapNat (1 / 2) 0 n)) ≤
        Twobites.paperP (1 / 2) n *
            (12 * (ε1 * Twobites.paperK (1 + ε) n ^ 2) +
              (⌈10 * (ε1 * Twobites.paperK (1 + ε) n ^ 2)⌉₊ : ℝ)) -
          Twobites.paperP (1 / 2) n *
            ((((xR + xB - 1) * (Twobites.paperKNat (1 + ε) n : ℝ)) *
              ...
              (Twobites.paperCapNat (1 / 2) 0 n : ℝ) *
                ((1 - xB) * (Twobites.paperKNat (1 + ε) n : ℝ) -
                  (Twobites.paperCapNat (1 / 2) 0 n : ℝ))) := by
    have hscaled := mul_le_mul_of_nonneg_left htargetLower hp0
    linarith
  have hresNonneg :
      0 ≤ (1 - xB) * (Twobites.paperKNat (1 + ε) n : ℝ) -
        (Twobites.paperCapNat (1 / 2) 0 n : ℝ) := by
    have hblueCapReal :
        ((C.blueImage I).card : ℝ) + (Twobites.paperCapNat (1 / 2) 0 n : ℝ) ≤
          (Twobites.paperKNat (1 + ε) n : ℝ) := by
      exact_mod_cast hblueCapNat
    rw [hblueEq] at hblueCapReal
    nlinarith
  have hxB1 : xB ≤ 1 := by
    have hs : xR + xB ≤ 1 + ε / 2 := hsumUpper
    have hblueHalf : xB ≤ (1 + ε / 2) / 2 := by
      linarith [hblueLe, hs]
    calc
      xB ≤ (1 + ε / 2) / 2 := hblueHalf
      _ = (1 / 2 : ℝ) + ε / 4 := by ring
      _ ≤ (1 / 2 : ℝ) + 1 / 16 := by
        have hquarter : ε / 4 ≤ (1 / 16 : ℝ) := by
          nlinarith [hεquarter]
        linarith
      _ ≤ 1 := by norm_num
  have hsection :
      (Twobites.paperP (1 / 2) n *
        ...
        (((-(1 / (4 * (1 + ε))) *
          ...
          Twobites.paperK (1 + ε) n * Real.log (n : ℝ)) := by
    exact hsection0.trans <|
      Twobites.paperRI_nearOne_sectionExp_le_of_budget hn hε hεquarter hδ hδle hdeltaLog
        hkone hε1pos.le hsumLower hsumUpper hxBpos.le hxB1 hresNonneg hbudget
  exact
    C.paper_ri_eqLong_bound_nearOne_of_chooseOuterEventMass_le_exp_of_mainRemainder_of_images_of_section_le
      hD I hindep hp0 hp1 hHsubset ht21 ht32 hn hε hεquarter hI hT2 hT1 hLChoose
      hLargeBound hMediumWitness hMediumBound hSmallCard hSmallBound hHChoose hRevealArith
      hLarge hMedium hSmall hHugeRed hHugeBlue hred hblue hblueCap hblueCapWeight hredCap
      hredCapWeight hρR hρB (by norm_num : 0 ≤ (1 / 2 : ℝ)) (by norm_num : -1 ≤ (0 : ℝ))
      hε1pos hε1le hloglogGap hdiagScale hcodegScale hsumGap hdegBound
      hchooseCodegBound hcodegBound hgap2R hκ2R hblueCrossSmall hgap2B hκ2B hredCrossSmall
