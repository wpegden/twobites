...
theorem paper_huge_deterministic_of_witnessErrorBounds_of_coeffBound_of_paramDeficit_of_split
  ...
  simpa [add_assoc, add_left_comm, add_comm] using
    C.paper_huge_deterministic_of_witnessErrorBounds_of_coeffBound_of_paramDeficit
      ...
      hbound hdegBound hcodegBound hsplit hδgapR hκ2R hblueCrossSmall hδgapB hκ2B
      hredCrossSmall
...
theorem paper_huge_deterministic_of_witnessErrorBounds_of_coeffBound_of_paramDeficit_of_three
  ...
  exact
    C.paper_huge_deterministic_of_witnessErrorBounds_of_coeffBound_of_paramDeficit_of_split
      ...
      hdiagRed hdiagBlue hε1 hbound hdegBound hcodegBound hsplitFirst hcodegCoeff
...
theorem paper_huge_deterministic_of_small_coeff_gap_diag_paperK_of_bound_le_of_witnessCoeff
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 δR δB βdeg qcodeg δsplit δwitness δsumGap δgapR δgapB B : ℝ}
    {bound witnessSize : ℕ}
    (hI : I.card ≤ Twobites.paperKNat κ n)
    (hwitness :
      Twobites.paperKNat κ n < witnessSize * ⌈Twobites.paperT1 n⌉₊ -
  ...
  have hκ : 0 ≤ κ := by
    have hfac : 0 ≤ 1 + ε2 := by linarith
    have hterm : 0 ≤ (1 + ε2) * β := mul_nonneg hfac hβ
    have hbase : 0 ≤ ρR + (1 + ε2) * β + δR := by
      exact add_nonneg (add_nonneg hρR hterm) hδR
    exact le_trans hbase hκR
  have hcoeff :
      δdeg ≤ (B * βdeg) / Twobites.paperS n := by
    dsimp [δdeg]
    exact
      div_le_div_of_nonneg_right (mul_le_mul_of_nonneg_right hbound hβdeg)
        (Twobites.paperS_nonneg n)
  have hdiag' :
      ((Twobites.paperK κ n + 1) / 2) * Twobites.paperK δdeg n ≤
        ε1 * Twobites.paperK κ n ^ 2 := by
    have hfac : 0 ≤ (Twobites.paperK κ n + 1) / 2 := by
      have hk : 0 ≤ Twobites.paperK κ n := Twobites.paperK_nonneg hκ n
      nlinarith
    have hmono :
        Twobites.paperK δdeg n ≤ Twobites.paperK (B * βdeg / Twobites.paperS n) n :=
      Twobites.paperK_le_paperK_of_le hcoeff
    calc
      ((Twobites.paperK κ n + 1) / 2) * Twobites.paperK δdeg n ≤
          ((Twobites.paperK κ n + 1) / 2) *
            Twobites.paperK (B * βdeg / Twobites.paperS n) n := by
              exact mul_le_mul_of_nonneg_left hmono hfac
      _ ≤ ε1 * Twobites.paperK κ n ^ 2 := hdiag
  have hsplit' : δdeg + δcodeg ≤ δsplit := by
    dsimp [δdeg, δcodeg]
    exact (Twobites.splitCoeff_le_of_bound_le hn hbound hβdeg hqcodeg).trans hsplit
  have hwitnessCoeff' : δdeg + δcodeg ≤ δwitness := by
    dsimp [δdeg, δcodeg]
    exact (Twobites.splitCoeff_le_of_bound_le hn hbound hβdeg hqcodeg).trans hwitnessCoeff
  have hcoeffNonneg : 0 ≤ δdeg + δcodeg := by
    simpa [δdeg, δcodeg] using
      (Twobites.splitCoeff_nonneg (bound := bound) (n := n) hβdeg hqcodeg)
  have hδsplit : 0 ≤ δsplit := le_trans hcoeffNonneg hsplit'
  have hδsumGap : 0 ≤ δsumGap := Twobites.nonneg_of_one_le_paperK hn hsumGap
  have hδerr : 0 ≤ δsplit + δsumGap := add_nonneg hδsplit hδsumGap
  have hsumκ :
      δdeg + δcodeg + δsumGap ≤ δsplit + δsumGap := by
    simpa [add_assoc, add_left_comm, add_comm] using add_le_add_right hsplit' δsumGap
  have hdegTerm :
  ...
  have hdiagNat :
      ((Twobites.paperKNat κ n : ℝ) / 2) * (witnessSize * degreeBound : ℕ) ≤
        ε1 * Twobites.paperK κ n ^ 2 := by
    have hδdegNonneg : 0 ≤ Twobites.paperK δdeg n := by
      apply Twobites.paperK_nonneg
      dsimp [δdeg]
      exact div_nonneg (mul_nonneg (Nat.cast_nonneg bound) hβdeg) (Twobites.paperS_nonneg n)
    have hceil :
        (Twobites.paperKNat κ n : ℝ) / 2 ≤ (Twobites.paperK κ n + 1) / 2 := by
      exact
        div_le_div_of_nonneg_right (Twobites.paperKNat_le_paperK_add_one hκ n)
          (by norm_num)
    calc
      ((Twobites.paperKNat κ n : ℝ) / 2) * (witnessSize * degreeBound : ℕ) ≤
          ((Twobites.paperKNat κ n : ℝ) / 2) * Twobites.paperK δdeg n := by
            exact mul_le_mul_of_nonneg_left hdegTerm (by positivity)
      _ ≤ ((Twobites.paperK κ n + 1) / 2) * Twobites.paperK δdeg n := by
        exact mul_le_mul_of_nonneg_right hceil hδdegNonneg
      _ ≤ ε1 * Twobites.paperK κ n ^ 2 := hdiag'
  have hblueCrossSmall' :
      (3 : ℝ) * (Twobites.paperK δdeg n + Twobites.paperK δcodeg n) ≤
        ε1 *
          (((Twobites.paperKNat κ n - Twobites.paperKNat ρR n -
              Twobites.paperCapNat β ε2 n : ℕ) : ℝ) - 1) := by
    have hmono : Twobites.paperK (δdeg + δcodeg) n ≤ Twobites.paperK δwitness n :=
      Twobites.paperK_le_paperK_of_le hwitnessCoeff'
    have hmono' :
        (3 : ℝ) * Twobites.paperK (δdeg + δcodeg) n ≤
          (3 : ℝ) * Twobites.paperK δwitness n := by
      exact mul_le_mul_of_nonneg_left hmono (by positivity)
    calc
      (3 : ℝ) * (Twobites.paperK δdeg n + Twobites.paperK δcodeg n) =
          (3 : ℝ) * Twobites.paperK (δdeg + δcodeg) n := by
            rw [← Twobites.paperK_add (n := n)]
      _ ≤ (3 : ℝ) * Twobites.paperK δwitness n := hmono'
      _ ≤ _ := hblueCrossSmall
  have hredCrossSmall' :
      (3 : ℝ) * (Twobites.paperK δdeg n + Twobites.paperK δcodeg n) ≤
        ε1 *
    ...
    calc
      ...
      _ ≤ _ := hredCrossSmall
  exact
    C.paper_huge_deterministic_of_split_degree_le_paperPM_of_codegree_le_of_split_small
      (δerr := δsplit + δsumGap) (bound := bound) hD I hI hwitness hred hblue hblueCap
      hblueCapWeight hredCap hredCapWeight hn hρR hρB hβ hε2 hgapR hgapB hκR hκB
      hdiagNat hdiagNat hε1 hw hβdeg hqcodeg hδerr hsumGap
      (by simpa [δdeg, δcodeg, add_assoc, add_left_comm, add_comm] using hsumκ) hdegBound
      hcodegBound hgap2R
      (by simpa [add_assoc, add_left_comm, add_comm] using hκ2R)
      (by simpa [δdeg, δcodeg] using hblueCrossSmall') hgap2B
      (by simpa [add_assoc, add_left_comm, add_comm] using hκ2B)
      (by simpa [δdeg, δcodeg] using hredCrossSmall')

theorem paper_huge_deterministic_of_small_coeff_gap_diag_paperK_of_witness_bound_le_of_witnessCoeff
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 δR δB βdeg qcodeg δsplit δwitness δsumGap δgapR δgapB B : ℝ}
        ...
        (((C.blueProjectionPairCount I ((C.HPart I).filter IsRedBaseVertex) : ℕ) : ℝ) ≤
          ...
          (((C.redProjectionPairCount I ((C.HPart I).filter IsBlueBaseVertex) : ℕ) : ℝ) ≤
              (1 + ε1) *
                ((C.paperHugeRedCrossTargetNat I κ (Twobites.paperCapNat β ε2 n) : ℕ) : ℝ)) := by
  exact
    C.paper_huge_deterministic_of_small_coeff_gap_diag_paperK_of_bound_le_of_witnessCoeff
      (bound := witnessSize) hD I hI hwitness hred hblue hblueCap hblueCapWeight hredCap
      hredCapWeight hn hρR hρB hβ hε2 hgapR hgapB hκR hκB hε1
      (le_rfl : witnessSize ≤ witnessSize) hbound hβdeg hqcodeg hdiag hsplit hwitnessCoeff
      hsumGap hdegBound hcodegBound hgap2R hκ2R hblueCrossSmall hgap2B hκ2B
      hredCrossSmall

set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_small_coeff_gap_diag_paperK_of_witness_bound_le_of_diagCoeff_of_witnessCoeff
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 δR δB βdeg qcodeg δsplit δwitness δsumGap δgapR δgapB B : ℝ}
  ...
  have hdiag :
    ...
    exact Twobites.half_add_one_mul_paperK_le_eps_mul_paperKSq_of_le hn hdiagCoeff
  exact
    C.paper_huge_deterministic_of_small_coeff_gap_diag_paperK_of_witness_bound_le_of_witnessCoeff
      hD I hI hwitness hred hblue hblueCap hblueCapWeight hredCap hredCapWeight hn hρR
      hρB hβ hε2 hgapR hgapB hκR hκB hε1 hbound hβdeg hqcodeg hdiag hsplit
...
theorem paper_huge_deterministic_of_witness_bound_of_diagCoeff_of_bounds_of_gaps_of_witnessCoeff
    ...
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 βdeg qcodeg δsplit δwitness δsumGap δgapR δgapB B : ℝ}
    {witnessSize : ℕ}
    (hI : I.card ≤ Twobites.paperKNat κ n)
    (hwitness :
      Twobites.paperKNat κ n < witnessSize * ⌈Twobites.paperT1 n⌉₊ -
...
theorem
  ...
  have hloglog : 0 < Real.log (Real.log (n : ℝ)) := by
    ...
    exact lt_of_lt_of_le hdivpos hloglogGap
  have hβdeg : 0 ≤ βdeg := Twobites.nonneg_of_le_paperP_mul_paperM hn hdegBound
  have hqcodeg : 0 ≤ qcodeg := Twobites.nonneg_of_natCast_le hcodegBound
  have hB0 :
...
set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_split_of_witnessCoeff
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB δcodeg δwitness : ℝ}
    (hI : I.card ≤ Twobites.paperKNat κ n)
    (hred : (C.redImage I).card ≤ Twobites.paperKNat ρR n)
    (hblue : (C.blueImage I).card ≤ Twobites.paperKNat ρB n)
    (hblueCap :
  ...
  have hB :
    ...
    have hconst : 2 + ε1 ≤ 3 := by linarith
    have hκloglog : 0 ≤ κ * Real.log (Real.log (n : ℝ)) := by
      exact mul_nonneg hκ0 (le_of_lt hloglog)
    calc
      (2 + ε1) * κ * Real.log (Real.log (n : ℝ)) =
          (2 + ε1) * (κ * Real.log (Real.log (n : ℝ))) := by ring
      _ ≤ 3 * (κ * Real.log (Real.log (n : ℝ))) := by
        exact mul_le_mul_of_nonneg_right hconst hκloglog
      _ = 3 * κ * Real.log (Real.log (n : ℝ)) := by ring
  have hbound :
      (Twobites.paperHugeWitnessNat κ n : ℝ) ≤
        3 * κ * Real.log (Real.log (n : ℝ)) := hboundBase.trans hB
  have hloglogOne : 1 ≤ Real.log (Real.log (n : ℝ)) := by
    have htwo : (2 : ℝ) ≤ 2 / ε1 := by
      refine (le_div_iff₀ hε1pos).2 ?_
      nlinarith
    have htwo' : (2 : ℝ) ≤ Real.log (Real.log (n : ℝ)) := le_trans htwo hloglogGap
    exact le_trans (by norm_num) htwo'
  have hk : 1 ≤ Twobites.paperK κ n := by
    have hT1leK : Twobites.paperT1 n ≤ Twobites.paperK κ n :=
      Twobites.paperT1_le_paperK hloglogOne hκ
    exact le_trans (by norm_num) <| le_trans (le_of_lt hT1) hT1leK
  have hdiagCoeff :
      ((3 * κ * Real.log (Real.log (n : ℝ))) * βdeg / Twobites.paperS n) *
          (Twobites.paperK κ n + 1) ≤
        2 * ε1 * κ * Twobites.paperK κ n := by
  ...
  have hsplit :
    ...
    have hfirst :
        ((3 * κ * Real.log (Real.log (n : ℝ))) * βdeg) / Twobites.paperS n ≤ ε1 * κ := by
      exact Twobites.three_loglog_split_first_le hn hκ0 hdiagScale
    exact add_le_add hfirst hcodegCoeff
  have hqcodeg : 0 ≤ qcodeg := Twobites.nonneg_of_natCast_le hcodegBound
  have hδsumGap : 0 ≤ δsumGap := Twobites.nonneg_of_one_le_paperK hn hsumGap
  have hgapR' : 1 ≤ Twobites.paperK δgapR n := le_trans (by norm_num) hgap2R
  have hδgapR : 0 ≤ δgapR := Twobites.nonneg_of_one_le_paperK hn hgapR'
  have hδcodeg_le_κ : δcodeg ≤ κ := by
    have hfac : 0 ≤ 1 + ε2 := by linarith
    have htermβ : 0 ≤ (1 + ε2) * β := mul_nonneg hfac hβ
    have htermκ : 0 ≤ ε1 * κ := mul_nonneg hε1 hκ0
    have hbase :
        0 ≤ ρR + (1 + ε2) * β + ε1 * κ + δsumGap + δgapR := by
      exact add_nonneg
        (add_nonneg (add_nonneg (add_nonneg hρR htermβ) htermκ) hδsumGap) hδgapR
    have hκ2R' :
        ρR + (1 + ε2) * β + ε1 * κ + δsumGap + δgapR + δcodeg ≤ κ := by
      simpa [add_assoc, add_left_comm, add_comm] using hκ2R
    have hle :
        δcodeg ≤ ρR + (1 + ε2) * β + ε1 * κ + δsumGap + δgapR + δcodeg := by
      simpa [add_assoc, add_left_comm, add_comm] using
        (le_add_of_nonneg_left hbase : δcodeg ≤
          (ρR + (1 + ε2) * β + ε1 * κ + δsumGap + δgapR) + δcodeg)
    exact hle.trans hκ2R'
  have hchooseCoeff :
      ((((3 * κ * Real.log (Real.log (n : ℝ))) ^ 2 / 2) * qcodeg) /
        Real.sqrt ((n : ℝ) * Real.log (n : ℝ))) ≤
      κ := hcodegCoeff.trans hδcodeg_le_κ
  have hchooseReal :
      (((Twobites.paperHugeWitnessNat κ n).choose 2 * codegreeBound : ℕ) : ℝ) ≤
        Twobites.paperK
          ((((3 * κ * Real.log (Real.log (n : ℝ))) ^ 2 / 2) * qcodeg) /
            Real.sqrt ((n : ℝ) * Real.log (n : ℝ))) n := by
    exact
      Twobites.cast_choose_mul_le_paperK_of_real_bound hbound hn hchooseCodegBound
  have hchooseReal' :
      (((Twobites.paperHugeWitnessNat κ n).choose 2 * codegreeBound : ℕ) : ℝ) ≤
        Twobites.paperK κ n := by
    exact hchooseReal.trans <| Twobites.paperK_le_paperK_of_le hchooseCoeff
  ...
  have hwitness :
    ...
    Twobites.paperKNat_lt_mul_ceil_paperT1_sub_choose_mul_of_two_mul_lt htwo hchoose
  exact
    C.paper_huge_deterministic_of_witness_bound_of_diagCoeff_of_bounds_of_gaps_of_witnessCoeff
      (B := 3 * κ * Real.log (Real.log (n : ℝ))) (δsplit := ε1 * κ + δcodeg) hD I hI
      hwitness hred hblue hblueCap hblueCapWeight hredCap hredCapWeight hn hρR hρB hβ
      hε2 hε1 hbound hdiagCoeff hsplit hwitnessCoeff hsumGap hdegBound hcodegBound hgap2R
      hκ2R hblueCrossSmall hgap2B hκ2B hredCrossSmall
...
set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_split_of_exactWitnessCoeff_of_coeffSmall
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB δcodeg δblue δred : ℝ}
    (hI : I.card ≤ Twobites.paperKNat κ n)
    (hred : (C.redImage I).card ≤ Twobites.paperKNat ρR n)
    (hblue : (C.blueImage I).card ≤ Twobites.paperKNat ρB n)
    (hblueCap :
    ...
    (hκ2R :
      ρR + (1 + ε2) * β + (ε1 * κ + δcodeg) + δsumGap + δgapR ≤ κ)
    (hblueCoeffSmall :
      Twobites.paperHugeWitnessCoeff κ βdeg qcodeg n ≤ (ε1 / 3) * δblue)
    (hblueResidual :
      Twobites.paperK δblue n ≤
        (((Twobites.paperKNat κ n - Twobites.paperKNat ρR n -
            Twobites.paperCapNat β ε2 n : ℕ) : ℝ) - 1))
    (hgap2B : 2 ≤ Twobites.paperK δgapB n)
    (hκ2B :
      ρB + (1 + ε2) * β + (ε1 * κ + δcodeg) + δsumGap + δgapB ≤ κ)
    (hredCoeffSmall :
      Twobites.paperHugeWitnessCoeff κ βdeg qcodeg n ≤ (ε1 / 3) * δred)
    (hredResidual :
      Twobites.paperK δred n ≤
        (((Twobites.paperKNat κ n - Twobites.paperKNat ρB n -
            Twobites.paperCapNat β ε2 n : ℕ) : ℝ) - 1)) :
    (((C.redProjectionPairCount I ((C.HPart I).filter IsRedBaseVertex) : ℕ) : ℝ) ≤
        ε1 * Twobites.paperK κ n ^ 2) ∧
      ...
      (3 : ℝ) * Twobites.paperK (Twobites.paperHugeWitnessCoeff κ βdeg qcodeg n) n ≤
        ε1 *
          (((Twobites.paperKNat κ n - Twobites.paperKNat ρR n -
              Twobites.paperCapNat β ε2 n : ℕ) : ℝ) - 1) := by
    exact
      Twobites.three_mul_paperK_paperHugeWitnessCoeff_le_of_le_mul_of_le
        hε1 hblueCoeffSmall hblueResidual
  have hredCrossSmall :
      (3 : ℝ) * Twobites.paperK (Twobites.paperHugeWitnessCoeff κ βdeg qcodeg n) n ≤
        ε1 *
          (((Twobites.paperKNat κ n - Twobites.paperKNat ρB n -
    ...
    exact
      Twobites.three_mul_paperK_paperHugeWitnessCoeff_le_of_le_mul_of_le
        hε1 hredCoeffSmall hredResidual
  exact
    C.paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_split_of_exactWitnessCoeff
      hD I hI hred hblue hblueCap hblueCapWeight hredCap hredCapWeight hn hκ hρR hρB
...
set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_witnessCoeff
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB δwitness : ℝ}
    (hI : I.card ≤ Twobites.paperKNat κ n)
    (hred : (C.redImage I).card ≤ Twobites.paperKNat ρR n)
    (hblue : (C.blueImage I).card ≤ Twobites.paperKNat ρB n)
    (hblueCap :
  ...
  simpa [hsplitEq, add_assoc, add_left_comm, add_comm] using
    C.paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_split_of_witnessCoeff
      (δcodeg := ε1 * κ) hD I hI hred hblue hblueCap hblueCapWeight hredCap
      hredCapWeight hn hκ hρR hρB hβ hε2 hT1 hε1pos hε1le hloglogGap hε1 hdiagScale
      hcodegScale hwitnessCoeff hsumGap hdegBound hchooseCodegBound hcodegBound hgap2R
      hκ2R' hblueCrossSmall hgap2B hκ2B' hredCrossSmall
...
set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_exactWitnessCoeff_of_coeffSmall
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB δblue δred : ℝ}
    (hI : I.card ≤ Twobites.paperKNat κ n)
    (hred : (C.redImage I).card ≤ Twobites.paperKNat ρR n)
    (hblue : (C.blueImage I).card ≤ Twobites.paperKNat ρB n)
    (hblueCap :
  ...
  have hκ2B' :
    ...
    simpa [hsplitEq, add_assoc, add_left_comm, add_comm] using hκ2B
  exact
    C.paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_split_of_exactWitnessCoeff_of_coeffSmall
      (δcodeg := ε1 * κ) hD I hI hred hblue hblueCap hblueCapWeight hredCap hredCapWeight
      hn hκ hρR hρB hβ hε2 hT1 hε1pos hε1le hloglogGap hε1 hdiagScale hcodegScale
      hsumGap hdegBound hchooseCodegBound hcodegBound hgap2R hκ2R' hblueCoeffSmall
      hblueResidual hgap2B hκ2B' hredCoeffSmall hredResidual

set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_exactWitnessCoeff_of_coeffSmall_of_paramDeficit
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB δblue δred : ℝ}
  ...
  have hκ2R :
      ρR + (1 + ε2) * β + 2 * ε1 * κ + δsumGap + δgapR ≤ κ := by
    have hdrop :
        ρR + (1 + ε2) * β + 2 * ε1 * κ + δsumGap + δgapR ≤
          ρR + (1 + ε2) * β + 2 * ε1 * κ + δblue + δsumGap + δgapR := by
      nlinarith
    exact hdrop.trans hκ3R
  have hκ2B :
      ρB + (1 + ε2) * β + 2 * ε1 * κ + δsumGap + δgapB ≤ κ := by
    have hdrop :
        ρB + (1 + ε2) * β + 2 * ε1 * κ + δsumGap + δgapB ≤
          ρB + (1 + ε2) * β + 2 * ε1 * κ + δred + δsumGap + δgapB := by
      nlinarith
    exact hdrop.trans hκ3B
  have hκ3R' :
      ρR + (1 + ε2) * β + (2 * ε1 * κ + δblue) + δsumGap + δgapR ≤ κ := by
    simpa [add_assoc, add_left_comm, add_comm] using hκ3R
  have hκ3B' :
      ρB + (1 + ε2) * β + (2 * ε1 * κ + δred) + δsumGap + δgapB ≤ κ := by
    simpa [add_assoc, add_left_comm, add_comm] using hκ3B
  have hdoubleBlue : 0 ≤ 2 * ε1 * κ + δblue := add_nonneg hdoubleEps hδblue
  have hdoubleRed : 0 ≤ 2 * ε1 * κ + δred := add_nonneg hdoubleEps hδred
  have hblueResidualAux :
      Twobites.paperK (2 * ε1 * κ + δblue) n ≤
        (((Twobites.paperKNat κ n - Twobites.paperKNat ρR n -
            Twobites.paperCapNat β ε2 n : ℕ) : ℝ) - 1) := by
    exact
      Twobites.paperK_le_cross_residual_sub_one_of_nonneg_of_gap1_gap2_of_le
        hn hρR hβ hε2 hdoubleBlue hsumGap hgap2R hκ3R'
  have hblueResidual :
      Twobites.paperK δblue n ≤
        (((Twobites.paperKNat κ n - Twobites.paperKNat ρR n -
            Twobites.paperCapNat β ε2 n : ℕ) : ℝ) - 1) := by
    have hmono :
        Twobites.paperK δblue n ≤ Twobites.paperK (2 * ε1 * κ + δblue) n := by
      exact Twobites.paperK_le_paperK_of_le (le_add_of_nonneg_left hdoubleEps)
    exact hmono.trans hblueResidualAux
  have hredResidualAux :
      Twobites.paperK (2 * ε1 * κ + δred) n ≤
        (((Twobites.paperKNat κ n - Twobites.paperKNat ρB n -
            Twobites.paperCapNat β ε2 n : ℕ) : ℝ) - 1) := by
    exact
      Twobites.paperK_le_cross_residual_sub_one_of_nonneg_of_gap1_gap2_of_le
        hn hρB hβ hε2 hdoubleRed hsumGap hgap2B hκ3B'
  have hredResidual :
      Twobites.paperK δred n ≤
        (((Twobites.paperKNat κ n - Twobites.paperKNat ρB n -
            Twobites.paperCapNat β ε2 n : ℕ) : ℝ) - 1) := by
    have hmono :
        Twobites.paperK δred n ≤ Twobites.paperK (2 * ε1 * κ + δred) n := by
      exact Twobites.paperK_le_paperK_of_le (le_add_of_nonneg_left hdoubleEps)
    exact hmono.trans hredResidualAux
  exact
    C.paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_exactWitnessCoeff_of_coeffSmall
      hD I hI hred hblue hblueCap hblueCapWeight hredCap hredCapWeight hn hκ hρR hρB
      hβ hε2 hT1 hε1pos hε1le hloglogGap hε1 hdiagScale hcodegScale hsumGap hdegBound
      hchooseCodegBound hcodegBound hgap2R hκ2R hblueCoeffSmall hblueResidual hgap2B
      hκ2B hredCoeffSmall hredResidual

set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_exactWitnessCoeff_of_exactBranchParam
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB : ℝ}
    ...
    (hκ3R :
      ρR + (1 + ε2) * β + 2 * ε1 * κ +
          Twobites.paperHugeWitnessBranchParam ε1 κ βdeg qcodeg n + δsumGap + δgapR ≤
        κ)
    (hgap2B : 2 ≤ Twobites.paperK δgapB n)
    (hκ3B :
      ρB + (1 + ε2) * β + 2 * ε1 * κ +
          Twobites.paperHugeWitnessBranchParam ε1 κ βdeg qcodeg n + δsumGap + δgapB ≤
        κ) :
    (((C.redProjectionPairCount I ((C.HPart I).filter IsRedBaseVertex) : ℕ) : ℝ) ≤
        ε1 * Twobites.paperK κ n ^ 2) ∧
      (((C.blueProjectionPairCount I ((C.HPart I).filter IsBlueBaseVertex) : ℕ) : ℝ) ≤
        ...
        (((C.blueProjectionPairCount I ((C.HPart I).filter IsRedBaseVertex) : ℕ) : ℝ) ≤
          ...
          (((C.redProjectionPairCount I ((C.HPart I).filter IsBlueBaseVertex) : ℕ) : ℝ) ≤
              (1 + ε1) *
                ((C.paperHugeRedCrossTargetNat I κ (Twobites.paperCapNat β ε2 n) : ℕ) : ℝ)) := by
  let δbranch : ℝ := Twobites.paperHugeWitnessBranchParam ε1 κ βdeg qcodeg n
  have hκ0 : 0 ≤ κ := by linarith
  have hβdeg : 0 ≤ βdeg := Twobites.nonneg_of_le_paperP_mul_paperM hn hdegBound
  have hqcodeg : 0 ≤ qcodeg := Twobites.nonneg_of_natCast_le hcodegBound
  have hloglog : 0 ≤ Real.log (Real.log (n : ℝ)) := by
    have hdivpos : 0 < 2 / ε1 := by positivity
    linarith
  have hδbranch : 0 ≤ δbranch := by
    dsimp [δbranch]
    exact Twobites.paperHugeWitnessBranchParam_nonneg hε1pos hκ0 hβdeg hqcodeg hloglog
  have hcoeffSmall :
      Twobites.paperHugeWitnessCoeff κ βdeg qcodeg n ≤ (ε1 / 3) * δbranch := by
    dsimp [δbranch]
    exact Twobites.paperHugeWitnessCoeff_le_eps_third_mul_branchParam hε1pos
  have hκ3R' :
      ρR + (1 + ε2) * β + 2 * ε1 * κ + δbranch + δsumGap + δgapR ≤ κ := by
    simpa [δbranch] using hκ3R
  have hκ3B' :
      ρB + (1 + ε2) * β + 2 * ε1 * κ + δbranch + δsumGap + δgapB ≤ κ := by
    simpa [δbranch] using hκ3B
  exact
    C.paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_exactWitnessCoeff_of_coeffSmall_of_paramDeficit
      (δblue := δbranch) (δred := δbranch) hD I hI hred hblue hblueCap hblueCapWeight
      hredCap hredCapWeight hn hκ hρR hρB hβ hε2 hT1 hε1pos hε1le hloglogGap hε1
      hdiagScale hcodegScale hsumGap hdegBound hchooseCodegBound hcodegBound hgap2R
      hcoeffSmall hδbranch hκ3R' hgap2B hcoeffSmall hδbranch hκ3B'

set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_exactPieceBranchParam
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB : ℝ}
  ...
  have hκ3R' :
      ρR + (1 + ε2) * β + 2 * ε1 * κ +
          Twobites.paperHugeWitnessBranchParam ε1 κ βdeg qcodeg n + δsumGap + δgapR ≤
        κ := by
    simpa [Twobites.paperHugeWitnessBranchParam_eq_add, add_assoc, add_left_comm, add_comm]
      using hκ3R
  have hκ3B' :
      ρB + (1 + ε2) * β + 2 * ε1 * κ +
          Twobites.paperHugeWitnessBranchParam ε1 κ βdeg qcodeg n + δsumGap + δgapB ≤
    ...
    simpa [Twobites.paperHugeWitnessBranchParam_eq_add, add_assoc, add_left_comm, add_comm]
      using hκ3B
  exact
    C.paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_exactWitnessCoeff_of_exactBranchParam
      hD I hI hred hblue hblueCap hblueCapWeight hredCap hredCapWeight hn hκ hρR hρB
      hβ hε2 hT1 hε1pos hε1le hloglogGap hε1 hdiagScale hcodegScale hsumGap hdegBound
...
set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_eventualBranchwisePieceBranchParamBounds
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB
      δdegBlue δcodegBlue δblue δdegRed δcodegRed δred : ℝ}
    (hI : I.card ≤ Twobites.paperKNat κ n)
    (hred : (C.redImage I).card ≤ Twobites.paperKNat ρR n)
    (hblue : (C.blueImage I).card ≤ Twobites.paperKNat ρB n)
    ...
    (hbranchwiseLarge :
      ∀ ⦃m : ℕ⦄, N ≤ m →
        Twobites.paperHugeWitnessDegreeBranchParam ε1 κ βdeg m ≤ δdegBlue ∧
          Twobites.paperHugeWitnessCodegBranchParam ε1 κ qcodeg m ≤ δcodegBlue ∧
            Twobites.paperHugeWitnessDegreeBranchParam ε1 κ βdeg m ≤ δdegRed ∧
              Twobites.paperHugeWitnessCodegBranchParam ε1 κ qcodeg m ≤ δcodegRed)
    (hN : N ≤ n)
    (hblueBranchSum : δdegBlue + δcodegBlue ≤ δblue)
    (hgap2R : 2 ≤ Twobites.paperK δgapR n)
    (hκ3R :
      ...
      (((C.blueProjectionPairCount I ((C.HPart I).filter IsBlueBaseVertex) : ℕ) : ℝ) ≤
        ...
        (((C.blueProjectionPairCount I ((C.HPart I).filter IsRedBaseVertex) : ℕ) : ℝ) ≤
          ...
          (((C.redProjectionPairCount I ((C.HPart I).filter IsBlueBaseVertex) : ℕ) : ℝ) ≤
              (1 + ε1) *
                ((C.paperHugeRedCrossTargetNat I κ (Twobites.paperCapNat β ε2 n) : ℕ) : ℝ)) := by
  rcases hbranchwiseLarge hN with ⟨hblueDegBound, hblueCodegBound, hredDegBound, hredCodegBound⟩
  exact
...
set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_eventualTwoEpsBranchPieceBounds_of_extraDeficit
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB δextraR δextraB : ℝ}
    (hI : I.card ≤ Twobites.paperKNat κ n)
    (hred : (C.redImage I).card ≤ Twobites.paperKNat ρR n)
    (hblue : (C.blueImage I).card ≤ Twobites.paperKNat ρB n)
    (hblueCap :
    ...
    (hbranchwiseLarge :
      ∀ ⦃m : ℕ⦄, N ≤ m →
        Twobites.paperHugeWitnessDegreeBranchParam ε1 κ βdeg m ≤ ε1 * κ ∧
          Twobites.paperHugeWitnessCodegBranchParam ε1 κ qcodeg m ≤ ε1 * κ ∧
            Twobites.paperHugeWitnessDegreeBranchParam ε1 κ βdeg m ≤ ε1 * κ ∧
              Twobites.paperHugeWitnessCodegBranchParam ε1 κ qcodeg m ≤ ε1 * κ)
    (hN : N ≤ n)
    (hgap2R : 2 ≤ Twobites.paperK δgapR n)
    (hextraR : 2 * ε1 * κ ≤ δextraR)
    (hκextraR :
      ρR + (1 + ε2) * β + 2 * ε1 * κ + δextraR + δsumGap + δgapR ≤ κ)
    (hgap2B : 2 ≤ Twobites.paperK δgapB n)
    (hextraB : 2 * ε1 * κ ≤ δextraB)
    (hκextraB :
      ρB + (1 + ε2) * β + 2 * ε1 * κ + δextraB + δsumGap + δgapB ≤ κ) :
    (((C.redProjectionPairCount I ((C.HPart I).filter IsRedBaseVertex) : ℕ) : ℝ) ≤
        ε1 * Twobites.paperK κ n ^ 2) ∧
      (((C.blueProjectionPairCount I ((C.HPart I).filter IsBlueBaseVertex) : ℕ) : ℝ) ≤
          ε1 * Twobites.paperK κ n ^ 2) ∧
    ...
    (((C.redProjectionPairCount I ((C.HPart I).filter IsBlueBaseVertex) : ℕ) : ℝ) ≤
              (1 + ε1) *
                ((C.paperHugeRedCrossTargetNat I κ (Twobites.paperCapNat β ε2 n) : ℕ) : ℝ)) := by
  have hκ4R :
      ρR + (1 + ε2) * β + 4 * ε1 * κ + δsumGap + δgapR ≤ κ := by
    exact
      Twobites.paperHugeWitness_four_eps_budget_of_extraDeficit hextraR <| by
        simpa [add_assoc, add_left_comm, add_comm] using hκextraR
  have hκ4B :
      ρB + (1 + ε2) * β + 4 * ε1 * κ + δsumGap + δgapB ≤ κ := by
    exact
      Twobites.paperHugeWitness_four_eps_budget_of_extraDeficit hextraB <| by
        simpa [add_assoc, add_left_comm, add_comm] using hκextraB
  have hbranchSum : ε1 * κ + ε1 * κ ≤ 2 * ε1 * κ := by
    exact le_of_eq (by ring)
  have hfour : 4 * ε1 * κ = 2 * ε1 * κ + 2 * ε1 * κ := by
    ring
  have hκ3R :
      ρR + (1 + ε2) * β + 2 * ε1 * κ + (2 * ε1 * κ) + δsumGap + δgapR ≤ κ := by
    simpa [hfour, add_assoc, add_left_comm, add_comm] using hκ4R
  have hκ3B :
      ρB + (1 + ε2) * β + 2 * ε1 * κ + (2 * ε1 * κ) + δsumGap + δgapB ≤ κ := by
    simpa [hfour, add_assoc, add_left_comm, add_comm] using hκ4B
  exact
    C.paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_eventualBranchwisePieceBranchParamBounds
      (δdegBlue := ε1 * κ) (δcodegBlue := ε1 * κ) (δblue := 2 * ε1 * κ)
      (δdegRed := ε1 * κ) (δcodegRed := ε1 * κ) (δred := 2 * ε1 * κ)
      hD I hI hred hblue hblueCap hblueCapWeight hredCap hredCapWeight hn hκ hρR hρB
      hβ hε2 hT1 hε1pos hε1le hloglogGap hε1 hdiagScale hcodegScale hsumGap hdegBound
      hchooseCodegBound hcodegBound N hbranchwiseLarge hN hbranchSum hgap2R hκ3R
      hbranchSum hgap2B hκ3B

set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_eventualTwoEpsBranchPieceBounds
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB : ℝ}
    ...
    (hgap2R : 2 ≤ Twobites.paperK δgapR n)
    (hκ4R :
      ρR + (1 + ε2) * β + 4 * ε1 * κ + δsumGap + δgapR ≤ κ)
    (hgap2B : 2 ≤ Twobites.paperK δgapB n)
    (hκ4B :
      ρB + (1 + ε2) * β + 4 * ε1 * κ + δsumGap + δgapB ≤ κ) :
    (((C.redProjectionPairCount I ((C.HPart I).filter IsRedBaseVertex) : ℕ) : ℝ) ≤
        ε1 * Twobites.paperK κ n ^ 2) ∧
      (((C.blueProjectionPairCount I ((C.HPart I).filter IsBlueBaseVertex) : ℕ) : ℝ) ≤
          ε1 * Twobites.paperK κ n ^ 2) ∧
  ...
  have hκextraB :
    ...
    simpa [hfour, add_assoc, add_left_comm, add_comm] using hκ4B
  exact
    C.paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_eventualTwoEpsBranchPieceBounds_of_extraDeficit
      (δextraR := 2 * ε1 * κ) (δextraB := 2 * ε1 * κ)
      hD I hI hred hblue hblueCap hblueCapWeight hredCap hredCapWeight hn hκ hρR hρB
      hβ hε2 hT1 hε1pos hε1le hloglogGap hε1 hdiagScale hcodegScale hsumGap
      hdegBound hchooseCodegBound hcodegBound N hbranchwiseLarge hN hgap2R
      (by linarith) hκextraR hgap2B (by linarith) hκextraB

set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_extraDeficit
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB δextraR δextraB : ℝ}
    ...
    (hcodegBound : (projCodegreeBound : ℝ) ≤ qcodeg)
    (hlarge :
      Twobites.paperHugeWitnessTwoEpsBranchPieceThreshold ε1 κ βdeg qcodeg ≤ n)
    (hgap2R : 2 ≤ Twobites.paperK δgapR n)
    (hextraR : 2 * ε1 * κ ≤ δextraR)
    (hκextraR :
      ρR + (1 + ε2) * β + 2 * ε1 * κ + δextraR + δsumGap + δgapR ≤ κ)
  ...
  have hβdeg : 0 ≤ βdeg := by
    exact Twobites.nonneg_of_le_paperP_mul_paperM (degreeBound := degreeBound) hn hdegBound
  have hqcodeg : 0 ≤ qcodeg := by
    exact le_trans (by exact_mod_cast Nat.zero_le codegreeBound) hchooseCodegBound
  have hbranchwiseLarge :
      ∀ ⦃m : ℕ⦄,
        Twobites.paperHugeWitnessTwoEpsBranchPieceThreshold ε1 κ βdeg qcodeg ≤ m →
          Twobites.paperHugeWitnessDegreeBranchParam ε1 κ βdeg m ≤ ε1 * κ ∧
            Twobites.paperHugeWitnessCodegBranchParam ε1 κ qcodeg m ≤ ε1 * κ ∧
              Twobites.paperHugeWitnessDegreeBranchParam ε1 κ βdeg m ≤ ε1 * κ ∧
                Twobites.paperHugeWitnessCodegBranchParam ε1 κ qcodeg m ≤ ε1 * κ := by
    intro m hm
    exact
      Twobites.paperHugeWitnessTwoEpsBranchPieceThreshold_spec hε1pos hκpos hβdeg hqcodeg hm
  exact
    C.paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_eventualTwoEpsBranchPieceBounds_of_extraDeficit
      hD I hI hred hblue hblueCap hblueCapWeight hredCap hredCapWeight hn hκ hρR hρB
      hβ hε2 hT1 hε1pos hε1le hloglogGap hε1 hdiagScale hcodegScale hsumGap
      hdegBound hchooseCodegBound hcodegBound
      (Twobites.paperHugeWitnessTwoEpsBranchPieceThreshold ε1 κ βdeg qcodeg)
      hbranchwiseLarge hlarge hgap2R hextraR hκextraR hgap2B hextraB hκextraB

set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_fourEpsBudget
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB : ℝ}
  ...
  have hκextraR :
      ρR + (1 + ε2) * β + 2 * ε1 * κ + (2 * ε1 * κ) + δsumGap + δgapR ≤ κ := by
    exact Twobites.paperHugeWitness_two_eps_budget_of_four_eps_budget hκ4R
  have hκextraB :
      ρB + (1 + ε2) * β + 2 * ε1 * κ + (2 * ε1 * κ) + δsumGap + δgapB ≤ κ := by
    exact Twobites.paperHugeWitness_two_eps_budget_of_four_eps_budget hκ4B
  exact
    C.paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_extraDeficit
      (δextraR := 2 * ε1 * κ) (δextraB := 2 * ε1 * κ)
      hD I hI hred hblue hblueCap hblueCapWeight hredCap hredCapWeight hn hκ hρR hρB
      hβ hε2 hT1 hε1pos hε1le hloglogGap hε1 hdiagScale hcodegScale hsumGap
      hdegBound hchooseCodegBound hcodegBound hlarge hgap2R (by linarith) hκextraR
      hgap2B (by linarith) hκextraB

set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_rhoBudget
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB : ℝ}
    ...
    (hgap2R : 2 ≤ Twobites.paperK δgapR n)
    (hρbudgetR :
      ρR ≤ Twobites.paperHugeWitnessFourEpsRhoBudget β ε1 ε2 κ δsumGap δgapR)
    (hgap2B : 2 ≤ Twobites.paperK δgapB n)
    (hρbudgetB :
      ρB ≤ Twobites.paperHugeWitnessFourEpsRhoBudget β ε1 ε2 κ δsumGap δgapB) :
    (((C.redProjectionPairCount I ((C.HPart I).filter IsRedBaseVertex) : ℕ) : ℝ) ≤
        ε1 * Twobites.paperK κ n ^ 2) ∧
      (((C.blueProjectionPairCount I ((C.HPart I).filter IsBlueBaseVertex) : ℕ) : ℝ) ≤
          ε1 * Twobites.paperK κ n ^ 2) ∧
  ...
  have hκ4R :
      ρR + (1 + ε2) * β + 4 * ε1 * κ + δsumGap + δgapR ≤ κ := by
    exact Twobites.paperHugeWitness_four_eps_budget_of_rho_le hρbudgetR
  have hκ4B :
      ρB + (1 + ε2) * β + 4 * ε1 * κ + δsumGap + δgapB ≤ κ := by
    exact Twobites.paperHugeWitness_four_eps_budget_of_rho_le hρbudgetB
  exact
    C.paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_fourEpsBudget
      hD I hI hred hblue hblueCap hblueCapWeight hredCap hredCapWeight hn hκ hρR hρB
      hβ hε2 hT1 hε1pos hε1le hloglogGap hε1 hdiagScale hcodegScale hsumGap
      hdegBound hchooseCodegBound hcodegBound hlarge hgap2R hκ4R hgap2B hκ4B

set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_coarseBudget
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB : ℝ}
  ...
  exact
    C.paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_extraDeficit
      (δextraR := 6 * κ) (δextraB := 6 * κ)
      hD I hI hred hblue hblueCap hblueCapWeight hredCap hredCapWeight hn hκ hρR hρB
      hβ hε2 hT1 hε1pos hε1le hloglogGap hε1 hdiagScale hcodegScale hsumGap
      hdegBound hchooseCodegBound hcodegBound hlarge hgap2R hextra hκ6R hgap2B hextra
      hκ6B

set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_paramDeficit
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB δblue δred : ℝ}
    ...
    (hlarge :
      Twobites.paperHugeWitnessTwoEpsBranchPieceThreshold ε1 κ βdeg qcodeg ≤ n)
    (hgap2R : 2 ≤ Twobites.paperK δgapR n)
    (hdoubleBlue : 2 * ε1 * κ ≤ δblue)
    (hκ3R :
      ρR + (1 + ε2) * β + 2 * ε1 * κ + δblue + δsumGap + δgapR ≤ κ)
    (hgap2B : 2 ≤ Twobites.paperK δgapB n)
    (hdoubleRed : 2 * ε1 * κ ≤ δred)
    (hκ3B :
      ρB + (1 + ε2) * β + 2 * ε1 * κ + δred + δsumGap + δgapB ≤ κ) :
    (((C.redProjectionPairCount I ((C.HPart I).filter IsRedBaseVertex) : ℕ) : ℝ) ≤
        ε1 * Twobites.paperK κ n ^ 2) ∧
  ...
  exact
    C.paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_extraDeficit
      (δextraR := δblue) (δextraB := δred)
      hD I hI hred hblue hblueCap hblueCapWeight hredCap hredCapWeight hn hκ hρR hρB
      hβ hε2 hT1 hε1pos hε1le hloglogGap hε1 hdiagScale hcodegScale hsumGap
      hdegBound hchooseCodegBound hcodegBound hlarge hgap2R hdoubleBlue hκ3R hgap2B
      hdoubleRed hκ3B
...
theorem
    ...
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB δbranch : ℝ}
    (hI : I.card ≤ Twobites.paperKNat κ n)
    (hred : (C.redImage I).card ≤ Twobites.paperKNat ρR n)
    (hblue : (C.blueImage I).card ≤ Twobites.paperKNat ρB n)
    (hblueCap :
    ...
    (hcodegBound : (projCodegreeBound : ℝ) ≤ qcodeg)
    (hbranchBound :
      Twobites.paperHugeWitnessBranchParam ε1 κ βdeg qcodeg n ≤ δbranch)
    (hgap2R : 2 ≤ Twobites.paperK δgapR n)
    (hκ3R :
      ρR + (1 + ε2) * β + 2 * ε1 * κ + δbranch + δsumGap + δgapR ≤ κ)
    (hgap2B : 2 ≤ Twobites.paperK δgapB n)
    (hκ3B :
      ρB + (1 + ε2) * β + 2 * ε1 * κ + δbranch + δsumGap + δgapB ≤ κ) :
    (((C.redProjectionPairCount I ((C.HPart I).filter IsRedBaseVertex) : ℕ) : ℝ) ≤
        ε1 * Twobites.paperK κ n ^ 2) ∧
      (((C.blueProjectionPairCount I ((C.HPart I).filter IsBlueBaseVertex) : ℕ) : ℝ) ≤
          ε1 * Twobites.paperK κ n ^ 2) ∧
  ...
  have hκ3R' :
    ...
    have hmono :
        ρR + (1 + ε2) * β + 2 * ε1 * κ +
            Twobites.paperHugeWitnessBranchParam ε1 κ βdeg qcodeg n + δsumGap + δgapR ≤
          ρR + (1 + ε2) * β + 2 * ε1 * κ + δbranch + δsumGap + δgapR := by
      nlinarith [hbranchBound]
    exact hmono.trans hκ3R
  have hκ3B' :
      ρB + (1 + ε2) * β + 2 * ε1 * κ +
          Twobites.paperHugeWitnessBranchParam ε1 κ βdeg qcodeg n + δsumGap + δgapB ≤
    ...
    have hmono :
        ρB + (1 + ε2) * β + 2 * ε1 * κ +
            Twobites.paperHugeWitnessBranchParam ε1 κ βdeg qcodeg n + δsumGap + δgapB ≤
          ρB + (1 + ε2) * β + 2 * ε1 * κ + δbranch + δsumGap + δgapB := by
      nlinarith [hbranchBound]
    exact hmono.trans hκ3B
  exact
    C.paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_exactWitnessCoeff_of_exactBranchParam
...
set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_branchCoeffSmall
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB δbranch : ℝ}
    ...
    (hcodegBound : (projCodegreeBound : ℝ) ≤ qcodeg)
    (hbranchCoeff :
      Twobites.paperHugeWitnessCoeff κ βdeg qcodeg n ≤ (ε1 / 3) * δbranch)
    (hgap2R : 2 ≤ Twobites.paperK δgapR n)
    (hκ3R :
      ρR + (1 + ε2) * β + 2 * ε1 * κ + δbranch + δsumGap + δgapR ≤ κ)
    (hgap2B : 2 ≤ Twobites.paperK δgapB n)
        ...
        (((C.blueProjectionPairCount I ((C.HPart I).filter IsRedBaseVertex) : ℕ) : ℝ) ≤
          ...
          (((C.redProjectionPairCount I ((C.HPart I).filter IsBlueBaseVertex) : ℕ) : ℝ) ≤
              (1 + ε1) *
                ((C.paperHugeRedCrossTargetNat I κ (Twobites.paperCapNat β ε2 n) : ℕ) : ℝ)) := by
  have hbranchBound :
      Twobites.paperHugeWitnessBranchParam ε1 κ βdeg qcodeg n ≤ δbranch := by
    exact Twobites.paperHugeWitnessBranchParam_le_of_coeff_le hε1pos hbranchCoeff
  exact
    C.paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_branchParamBound
      hD I hI hred hblue hblueCap hblueCapWeight hredCap hredCapWeight hn hκ hρR hρB
      hβ hε2 hT1 hε1pos hε1le hloglogGap hε1 hdiagScale hcodegScale hsumGap hdegBound
      hchooseCodegBound hcodegBound hbranchBound hgap2R hκ3R hgap2B hκ3B

set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_pieceCoeffSmall
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB δdeg δcodeg δbranch : ℝ}
    (hI : I.card ≤ Twobites.paperKNat κ n)
    (hred : (C.redImage I).card ≤ Twobites.paperKNat ρR n)
    (hblue : (C.blueImage I).card ≤ Twobites.paperKNat ρB n)
    (hblueCap :
    ...
    (hsecondCoeffSmall :
      ((((3 * κ * Real.log (Real.log (n : ℝ))) ^ 2 / 2) * qcodeg) /
        Real.sqrt ((n : ℝ) * Real.log (n : ℝ))) ≤
      (ε1 / 3) * δcodeg)
    (hbranchSum : δdeg + δcodeg ≤ δbranch)
    (hgap2R : 2 ≤ Twobites.paperK δgapR n)
    (hκ3R :
      ρR + (1 + ε2) * β + 2 * ε1 * κ + δbranch + δsumGap + δgapR ≤ κ)
    (hgap2B : 2 ≤ Twobites.paperK δgapB n)
  ...
  have hbranchCoeff :
    ...
    exact
      Twobites.paperHugeWitnessCoeff_le_eps_third_mul_of_piece_bounds
        hε1 hfirstCoeffSmall hsecondCoeffSmall hbranchSum
  exact
    C.paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_branchCoeffSmall
      hD I hI hred hblue hblueCap hblueCapWeight hredCap hredCapWeight hn hκ hρR hρB
      hβ hε2 hT1 hε1pos hε1le hloglogGap hε1 hdiagScale hcodegScale hsumGap hdegBound
      hchooseCodegBound hcodegBound hbranchCoeff hgap2R hκ3R hgap2B hκ3B

set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_branchwisePieceCoeffSmall
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB : ℝ}
    ...
    (hchooseCodegBound : (codegreeBound : ℝ) ≤ qcodeg)
    (hcodegBound : (projCodegreeBound : ℝ) ≤ qcodeg)
    (hblueFirstCoeffSmall :
      ((3 * κ * Real.log (Real.log (n : ℝ))) * βdeg) / Twobites.paperS n ≤
        (ε1 / 3) * δdegBlue)
    (hblueSecondCoeffSmall :
      ((((3 * κ * Real.log (Real.log (n : ℝ))) ^ 2 / 2) * qcodeg) /
        Real.sqrt ((n : ℝ) * Real.log (n : ℝ))) ≤
      (ε1 / 3) * δcodegBlue)
    (hblueBranchSum : δdegBlue + δcodegBlue ≤ δblue)
...
set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_kSmall_of_witnessCoeff
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB δwitness : ℝ}
  ...
  exact
    C.paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_witnessCoeff
      hD I hI hred hblue hblueCap hblueCapWeight hredCap hredCapWeight hn hκ hρR hρB
      hβ hε2 hT1 hε1pos hε1le hloglogGap hε1 hdiagScale hcodegScale hwitnessCoeff hsumGap
      hdegBound hchooseCodegBound hcodegBound hgap2R hκ2R hblueCrossSmall' hgap2B
      hκ2B hredCrossSmall'

set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_kSmall_of_exactWitnessCoeff
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB : ℝ}
...
set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_witnessCoeff_of_paramDeficit
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB δwitness δblue δred : ℝ}
    (hI : I.card ≤ Twobites.paperKNat κ n)
    (hred : (C.redImage I).card ≤ Twobites.paperKNat ρR n)
    (hblue : (C.blueImage I).card ≤ Twobites.paperKNat ρB n)
    (hblueCap :
        ...
        (((C.blueProjectionPairCount I ((C.HPart I).filter IsRedBaseVertex) : ℕ) : ℝ) ≤
          ...
          (((C.redProjectionPairCount I ((C.HPart I).filter IsBlueBaseVertex) : ℕ) : ℝ) ≤
              (1 + ε1) *
                ((C.paperHugeRedCrossTargetNat I κ (Twobites.paperCapNat β ε2 n) : ℕ) : ℝ)) := by
  exact
    C.paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_paramDeficit
      hD I hI hred hblue hblueCap hblueCapWeight hredCap hredCapWeight hn hκ hρR hρB
      hβ hε2 hT1 hε1pos hε1le hloglogGap hε1 hdiagScale hcodegScale hsumGap
      hdegBound hchooseCodegBound hcodegBound hlarge hgap2R hdoubleBlue hκ3R hgap2B
      hdoubleRed hκ3B
...
set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_exactWitnessCoeff_of_paramDeficit
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB δblue δred : ℝ}
...
set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_witnessCoeff_of_rhoBudget
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB δwitness : ℝ}
        ...
        (((C.blueProjectionPairCount I ((C.HPart I).filter IsRedBaseVertex) : ℕ) : ℝ) ≤
          ...
          (((C.redProjectionPairCount I ((C.HPart I).filter IsBlueBaseVertex) : ℕ) : ℝ) ≤
              (1 + ε1) *
                ((C.paperHugeRedCrossTargetNat I κ (Twobites.paperCapNat β ε2 n) : ℕ) : ℝ)) := by
  exact
    C.paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_rhoBudget
      hD I hI hred hblue hblueCap hblueCapWeight hredCap hredCapWeight hn hκ hρR hρB
      hβ hε2 hT1 hε1pos hε1le hloglogGap hε1 hdiagScale hcodegScale hsumGap
      hdegBound hchooseCodegBound hcodegBound hlarge hgap2R hρbudgetR hgap2B hρbudgetB

set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_witnessCoeff_of_fourEpsBudget
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB δwitness : ℝ}
...
set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_exactWitnessCoeff_of_rhoBudget
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB : ℝ}
...
set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_exactWitnessCoeff_of_fourEpsBudget
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB β κ ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB : ℝ}
...
set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_halfBeta_of_onePlusEps_of_quarterProjection
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB ε ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB : ℝ}
    (hI : I.card ≤ Twobites.paperKNat (1 + ε) n)
    (hred : (C.redImage I).card ≤ Twobites.paperKNat ρR n)
    (hblue : (C.blueImage I).card ≤ Twobites.paperKNat ρB n)
    (hblueCap :
      ∀ x ∈ (C.HPart I).filter IsRedBaseVertex,
        (C.blueProjectionImage I x).card ≤ Twobites.paperCapNat (1 / 2) ε2 n)
    (hblueCapWeight :
      Twobites.paperCapNat (1 / 2) ε2 n ≤
        C.blueProjectionWeight I ((C.HPart I).filter IsRedBaseVertex))
    (hredCap :
      ∀ x ∈ (C.HPart I).filter IsBlueBaseVertex,
        (C.redProjectionImage I x).card ≤ Twobites.paperCapNat (1 / 2) ε2 n)
    (hredCapWeight :
      Twobites.paperCapNat (1 / 2) ε2 n ≤
        C.redProjectionWeight I ((C.HPart I).filter IsBlueBaseVertex))
    (hn : 1 < n) (hε : 0 ≤ ε)
    (hρR : 0 ≤ ρR) (hρB : 0 ≤ ρB) (hε2 : -1 ≤ ε2)
    (hT1 : 2 < Twobites.paperT1 n) (hε1pos : 0 < ε1) (hε1le : ε1 ≤ 1)
    (hloglogGap : 2 / ε1 ≤ Real.log (Real.log (n : ℝ)))
    (hε1 : 0 ≤ ε1)
    (hdiagScale :
    ...
    (hcodegBound : (projCodegreeBound : ℝ) ≤ qcodeg)
    (hlarge :
      Twobites.paperHugeWitnessTwoEpsBranchPieceThreshold ε1 (1 + ε) βdeg qcodeg ≤ n)
    (hgap2R : 2 ≤ Twobites.paperK δgapR n)
    (hρquarterR : ρR ≤ (((1 / 2 : ℝ) + ε / 4) * (1 + ε)))
    (hquarterGapR :
      ε2 / 2 + 4 * ε1 * (1 + ε) + δsumGap + δgapR ≤ ε * (1 - ε) / 4)
    (hgap2B : 2 ≤ Twobites.paperK δgapB n)
    (hρquarterB : ρB ≤ (((1 / 2 : ℝ) + ε / 4) * (1 + ε)))
    (hquarterGapB :
      ε2 / 2 + 4 * ε1 * (1 + ε) + δsumGap + δgapB ≤ ε * (1 - ε) / 4) :
    (((C.redProjectionPairCount I ((C.HPart I).filter IsRedBaseVertex) : ℕ) : ℝ) ≤
    ...
    (((C.blueProjectionPairCount I ((C.HPart I).filter IsBlueBaseVertex) : ℕ) : ℝ) ≤
        ...
        (((C.blueProjectionPairCount I ((C.HPart I).filter IsRedBaseVertex) : ℕ) : ℝ) ≤
            (1 + ε1) *
              ((C.paperHugeBlueCrossTargetNat I (1 + ε)
                  (Twobites.paperCapNat (1 / 2) ε2 n) : ℕ) : ℝ)) ∧
          (((C.redProjectionPairCount I ((C.HPart I).filter IsBlueBaseVertex) : ℕ) : ℝ) ≤
              (1 + ε1) *
                ((C.paperHugeRedCrossTargetNat I (1 + ε)
                    (Twobites.paperCapNat (1 / 2) ε2 n) : ℕ) : ℝ)) := by
  have hκ : 1 ≤ 1 + ε := by
    linarith
  have hβ : 0 ≤ (1 / 2 : ℝ) := by
    norm_num
  have hρbudgetR :
      ρR ≤ Twobites.paperHugeWitnessFourEpsRhoBudget
        (1 / 2) ε1 ε2 (1 + ε) δsumGap δgapR := by
    exact
      Twobites.paperHugeWitnessFourEpsRhoBudget_half_one_add_eps_of_rho_le
        hρquarterR hquarterGapR
  have hρbudgetB :
      ρB ≤ Twobites.paperHugeWitnessFourEpsRhoBudget
        (1 / 2) ε1 ε2 (1 + ε) δsumGap δgapB := by
    exact
      Twobites.paperHugeWitnessFourEpsRhoBudget_half_one_add_eps_of_rho_le
        hρquarterB hquarterGapB
  exact
    C.paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_rhoBudget
      hD I hI hred hblue hblueCap hblueCapWeight hredCap hredCapWeight hn hκ hρR hρB
      hβ hε2 hT1 hε1pos hε1le hloglogGap hε1 hdiagScale hcodegScale hsumGap
...
set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_witnessCoeff_of_halfBeta_of_onePlusEps_of_quarterProjection
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB ε ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB δwitness : ℝ}
    (hI : I.card ≤ Twobites.paperKNat (1 + ε) n)
    (hred : (C.redImage I).card ≤ Twobites.paperKNat ρR n)
    (hblue : (C.blueImage I).card ≤ Twobites.paperKNat ρB n)
    (hblueCap :
    ...
    (hcodegScale :
      ...
      ε1 * (1 + ε))
    (_hwitnessCoeff :
      ((3 * (1 + ε) * Real.log (Real.log (n : ℝ))) * βdeg) / Twobites.paperS n +
          ((((3 * (1 + ε) * Real.log (Real.log (n : ℝ))) ^ 2 / 2) * qcodeg) /
            Real.sqrt ((n : ℝ) * Real.log (n : ℝ))) ≤
        δwitness)
    (_hδwitness : δwitness ≤ 2 * ε1 * (1 + ε))
    (hsumGap : 1 ≤ Twobites.paperK δsumGap n)
    (hdegBound : (degreeBound : ℝ) ≤ Twobites.paperP βdeg n * Twobites.paperM n)
    (hchooseCodegBound : (codegreeBound : ℝ) ≤ qcodeg)
    (hcodegBound : (projCodegreeBound : ℝ) ≤ qcodeg)
          ...
          (((C.redProjectionPairCount I ((C.HPart I).filter IsBlueBaseVertex) : ℕ) : ℝ) ≤
              (1 + ε1) *
                ((C.paperHugeRedCrossTargetNat I (1 + ε)
                    (Twobites.paperCapNat (1 / 2) ε2 n) : ℕ) : ℝ)) := by
  exact
    C.paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_halfBeta_of_onePlusEps_of_quarterProjection
      hD I hI hred hblue hblueCap hblueCapWeight hredCap hredCapWeight hn hε hρR hρB
      hε2 hT1 hε1pos hε1le hloglogGap hε1 hdiagScale hcodegScale hsumGap hdegBound
      hchooseCodegBound hcodegBound hlarge hgap2R hρquarterR hquarterGapR hgap2B
      hρquarterB hquarterGapB

set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_exactWitnessCoeff_of_halfBeta_of_onePlusEps_of_quarterProjection
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB ε ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB : ℝ}
...
set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_halfBeta_of_onePlusEps_of_blueQuarterProjection_of_redKSmall
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB ε ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB : ℝ}
    ...
    (hgap2B : 2 ≤ Twobites.paperK δgapB n)
    (hκ2B :
      ρB + (1 + ε2) * (1 / 2) + 2 * ε1 * (1 + ε) + δsumGap + δgapB ≤ 1 + ε)
    (hredCrossSmall :
      6 * Twobites.paperK (1 + ε) n ≤
        (((Twobites.paperKNat (1 + ε) n - Twobites.paperKNat ρB n -
            Twobites.paperCapNat (1 / 2) ε2 n : ℕ) : ℝ) - 1)) :
    (((C.redProjectionPairCount I ((C.HPart I).filter IsRedBaseVertex) : ℕ) : ℝ) ≤
        ε1 * Twobites.paperK (1 + ε) n ^ 2) ∧
    (((C.blueProjectionPairCount I ((C.HPart I).filter IsBlueBaseVertex) : ℕ) : ℝ) ≤
          ε1 * Twobites.paperK (1 + ε) n ^ 2) ∧
        (((C.blueProjectionPairCount I ((C.HPart I).filter IsRedBaseVertex) : ℕ) : ℝ) ≤
          ...
          (((C.redProjectionPairCount I ((C.HPart I).filter IsBlueBaseVertex) : ℕ) : ℝ) ≤
              (1 + ε1) *
                ((C.paperHugeRedCrossTargetNat I (1 + ε)
                    (Twobites.paperCapNat (1 / 2) ε2 n) : ℕ) : ℝ)) := by
  have hκ : 1 ≤ 1 + ε := by linarith
  have hκ0 : 0 ≤ 1 + ε := by linarith
  have hβ : 0 ≤ (1 / 2 : ℝ) := by norm_num
  have hβdeg : 0 ≤ βdeg := Twobites.nonneg_of_le_paperP_mul_paperM hn hdegBound
  have hqcodeg : 0 ≤ qcodeg := Twobites.nonneg_of_natCast_le hcodegBound
  have hρbudgetR :
      ρR ≤ Twobites.paperHugeWitnessFourEpsRhoBudget
    ...
    exact
      Twobites.paperHugeWitnessFourEpsRhoBudget_half_one_add_eps_of_rho_le
        hρquarterR hquarterGapR
  have hκ4R :
      ρR + (1 + ε2) * (1 / 2) + 4 * ε1 * (1 + ε) + δsumGap + δgapR ≤ 1 + ε := by
    exact Twobites.paperHugeWitness_four_eps_budget_of_rho_le hρbudgetR
  have hκ2R :
      ρR + (1 + ε2) * (1 / 2) + 2 * ε1 * (1 + ε) + δsumGap + δgapR ≤ 1 + ε := by
    nlinarith
  have hblueCrossSmall :
      (3 : ℝ) * Twobites.paperK (Twobites.paperHugeWitnessCoeff (1 + ε) βdeg qcodeg n) n ≤
        ε1 *
          (((Twobites.paperKNat (1 + ε) n - Twobites.paperKNat ρR n -
              Twobites.paperCapNat (1 / 2) ε2 n : ℕ) : ℝ) - 1) := by
    exact
      Twobites.paperHugeWitness_crossSmall_of_twoEpsThreshold_of_rhoBudget
        hn hκ hρR hβ hε2 hε1pos hε1 hβdeg hqcodeg hlarge hsumGap hgap2R hρbudgetR
  have hδwitness :
      Twobites.paperHugeWitnessCoeff (1 + ε) βdeg qcodeg n ≤ 2 * ε1 * (1 + ε) := by
    simpa using
      Twobites.three_loglog_witnessCoeff_le_two_eps_mul hn hκ0 hdiagScale hcodegScale
  have hredCrossSmall' :
      (3 : ℝ) * Twobites.paperK (Twobites.paperHugeWitnessCoeff (1 + ε) βdeg qcodeg n) n ≤
...
set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_witnessCoeff_of_halfBeta_of_onePlusEps_of_blueQuarterProjection_of_redKSmall
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB ε ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB δwitness : ℝ}
          ...
          (((C.redProjectionPairCount I ((C.HPart I).filter IsBlueBaseVertex) : ℕ) : ℝ) ≤
              (1 + ε1) *
                ((C.paperHugeRedCrossTargetNat I (1 + ε)
                    (Twobites.paperCapNat (1 / 2) ε2 n) : ℕ) : ℝ)) := by
  exact
    C.paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_halfBeta_of_onePlusEps_of_blueQuarterProjection_of_redKSmall
      hD I hI hred hblue hblueCap hblueCapWeight hredCap hredCapWeight hn hε hρR hρB
      hε2 hT1 hε1pos hε1le hloglogGap hε1 hdiagScale hcodegScale hsumGap hdegBound
      hchooseCodegBound hcodegBound hlarge hgap2R hρquarterR hquarterGapR hgap2B hκ2B
      hredCrossSmall

set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_exactWitnessCoeff_of_halfBeta_of_onePlusEps_of_blueQuarterProjection_of_redKSmall
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB ε ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB : ℝ}
...
set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_halfBeta_of_onePlusEps_of_blueKSmall_of_redQuarterProjection
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB ε ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB : ℝ}
    ...
    (hgap2R : 2 ≤ Twobites.paperK δgapR n)
    (hκ2R :
      ρR + (1 + ε2) * (1 / 2) + 2 * ε1 * (1 + ε) + δsumGap + δgapR ≤ 1 + ε)
    (hblueCrossSmall :
      6 * Twobites.paperK (1 + ε) n ≤
        (((Twobites.paperKNat (1 + ε) n - Twobites.paperKNat ρR n -
            Twobites.paperCapNat (1 / 2) ε2 n : ℕ) : ℝ) - 1))
  ...
  have hρbudgetB :
    ...
    exact
      Twobites.paperHugeWitnessFourEpsRhoBudget_half_one_add_eps_of_rho_le
        hρquarterB hquarterGapB
  have hκ4B :
      ρB + (1 + ε2) * (1 / 2) + 4 * ε1 * (1 + ε) + δsumGap + δgapB ≤ 1 + ε := by
    exact Twobites.paperHugeWitness_four_eps_budget_of_rho_le hρbudgetB
  have hκ2B :
      ρB + (1 + ε2) * (1 / 2) + 2 * ε1 * (1 + ε) + δsumGap + δgapB ≤ 1 + ε := by
    nlinarith
  have hredCrossSmall :
      (3 : ℝ) * Twobites.paperK (Twobites.paperHugeWitnessCoeff (1 + ε) βdeg qcodeg n) n ≤
        ε1 *
    ...
    exact
      Twobites.paperHugeWitness_crossSmall_of_twoEpsThreshold_of_rhoBudget
        hn hκ hρB hβ hε2 hε1pos hε1 hβdeg hqcodeg hlarge hsumGap hgap2B hρbudgetB
  have hδwitness :
      Twobites.paperHugeWitnessCoeff (1 + ε) βdeg qcodeg n ≤ 2 * ε1 * (1 + ε) := by
    simpa using
      Twobites.three_loglog_witnessCoeff_le_two_eps_mul hn hκ0 hdiagScale hcodegScale
...
set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_witnessCoeff_of_halfBeta_of_onePlusEps_of_blueKSmall_of_redQuarterProjection
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB ε ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB δwitness : ℝ}
          ...
          (((C.redProjectionPairCount I ((C.HPart I).filter IsBlueBaseVertex) : ℕ) : ℝ) ≤
              (1 + ε1) *
                ((C.paperHugeRedCrossTargetNat I (1 + ε)
                    (Twobites.paperCapNat (1 / 2) ε2 n) : ℕ) : ℝ)) := by
  exact
    C.paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_halfBeta_of_onePlusEps_of_blueKSmall_of_redQuarterProjection
      hD I hI hred hblue hblueCap hblueCapWeight hredCap hredCapWeight hn hε hρR hρB
      hε2 hT1 hε1pos hε1le hloglogGap hε1 hdiagScale hcodegScale hsumGap hdegBound
      hchooseCodegBound hcodegBound hlarge hgap2R hκ2R hblueCrossSmall hgap2B hρquarterB
      hquarterGapB

set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_exactWitnessCoeff_of_halfBeta_of_onePlusEps_of_blueKSmall_of_redQuarterProjection
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB ε ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB : ℝ}
...
set_option linter.style.longLine false in
/-- Paper Lemma `lem:RI`, middle regime after relabeling so the smaller branch is `B`.
This is the paper-aligned asymmetric huge-case route: the `B`-branch gets the quarter-projection
budget coming from `x_B ≤ (x_R + x_B) / 2 < 1/2 + ε/4`, while the opposite branch still uses the
surviving coarse `kSmall` control. -/
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_halfBeta_of_onePlusEps_of_middleRange_of_blueLeRed
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB ε ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB : ℝ}
        ...
        (((C.blueProjectionPairCount I ((C.HPart I).filter IsRedBaseVertex) : ℕ) : ℝ) ≤
          ...
          (((C.redProjectionPairCount I ((C.HPart I).filter IsBlueBaseVertex) : ℕ) : ℝ) ≤
              (1 + ε1) *
                ((C.paperHugeRedCrossTargetNat I (1 + ε)
                    (Twobites.paperCapNat (1 / 2) ε2 n) : ℕ) : ℝ)) := by
  have hρquarterB : ρB ≤ (((1 / 2 : ℝ) + ε / 4) * (1 + ε)) := by
    exact
      Twobites.paperHugeWitnessQuarterProjection_of_middleSum_of_blueLeRed
        hblueLeRed hsumMiddle
  exact
    C.paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_halfBeta_of_onePlusEps_of_blueKSmall_of_redQuarterProjection
      hD I hI hred hblue hblueCap hblueCapWeight hredCap hredCapWeight hn hε hρR hρB
      hε2 hT1 hε1pos hε1le hloglogGap hε1 hdiagScale hcodegScale hsumGap hdegBound
...
set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_witnessCoeff_of_halfBeta_of_onePlusEps_of_middleRange_of_blueLeRed
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB ε ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB δwitness : ℝ}
          ...
          (((C.redProjectionPairCount I ((C.HPart I).filter IsBlueBaseVertex) : ℕ) : ℝ) ≤
              (1 + ε1) *
                ((C.paperHugeRedCrossTargetNat I (1 + ε)
                    (Twobites.paperCapNat (1 / 2) ε2 n) : ℕ) : ℝ)) := by
  exact
    C.paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_halfBeta_of_onePlusEps_of_middleRange_of_blueLeRed
      hD I hI hred hblue hblueCap hblueCapWeight hredCap hredCapWeight hn hε hρR hρB
      hε2 hT1 hε1pos hε1le hloglogGap hε1 hdiagScale hcodegScale hsumGap hdegBound
      hchooseCodegBound hcodegBound hlarge hgap2R hκ2R hblueCrossSmall hgap2B
      hblueLeRed hsumMiddle hquarterGapB

set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_exactWitnessCoeff_of_halfBeta_of_onePlusEps_of_middleRange_of_blueLeRed
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB ε ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB : ℝ}
...
set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_halfBeta_of_onePlusEps_of_xMiddleRange_of_blueLeRed
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB xR xB ε ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB : ℝ}
    (hI : I.card ≤ Twobites.paperKNat (1 + ε) n)
    (hred : (C.redImage I).card ≤ Twobites.paperKNat ρR n)
    (hblue : (C.blueImage I).card ≤ Twobites.paperKNat ρB n)
    (hblueCap :
  ...
  have hρquarterB : ρB ≤ (((1 / 2 : ℝ) + ε / 4) * (1 + ε)) := by
    exact
      Twobites.paperHugeWitnessQuarterProjection_of_xMiddleSum_of_blueLeRed
        hε hρBx hblueLeRed hxsumMiddle
  exact
    C.paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_halfBeta_of_onePlusEps_of_blueKSmall_of_redQuarterProjection
      hD I hI hred hblue hblueCap hblueCapWeight hredCap hredCapWeight hn hε hρR hρB
      hε2 hT1 hε1pos hε1le hloglogGap hε1 hdiagScale hcodegScale hsumGap hdegBound
...
set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_witnessCoeff_of_halfBeta_of_onePlusEps_of_xMiddleRange_of_blueLeRed
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB xR xB ε ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB δwitness : ℝ}
    (hI : I.card ≤ Twobites.paperKNat (1 + ε) n)
    (hred : (C.redImage I).card ≤ Twobites.paperKNat ρR n)
    (hblue : (C.blueImage I).card ≤ Twobites.paperKNat ρB n)
    (hblueCap :
          ...
          (((C.redProjectionPairCount I ((C.HPart I).filter IsBlueBaseVertex) : ℕ) : ℝ) ≤
              (1 + ε1) *
                ((C.paperHugeRedCrossTargetNat I (1 + ε)
                    (Twobites.paperCapNat (1 / 2) ε2 n) : ℕ) : ℝ)) := by
  exact
    C.paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_twoEpsBranchPieceThreshold_of_halfBeta_of_onePlusEps_of_xMiddleRange_of_blueLeRed
      hD I hI hred hblue hblueCap hblueCapWeight hredCap hredCapWeight hn hε hρR hρB
      hε2 hT1 hε1pos hε1le hloglogGap hε1 hdiagScale hcodegScale hsumGap hdegBound
      hchooseCodegBound hcodegBound hlarge hgap2R hκ2R hblueCrossSmall hgap2B hρBx
      hblueLeRed hxsumMiddle hquarterGapB

set_option linter.style.longLine false in
theorem
    paper_huge_deterministic_of_paperHugeWitness_of_eps1Slack_of_three_of_diagScale_of_codegScale_of_doubleEps_of_exactWitnessCoeff_of_halfBeta_of_onePlusEps_of_xMiddleRange_of_blueLeRed
    (C : ConstructionData n m) {fiberBound degreeBound codegreeBound projCodegreeBound : ℕ}
    (hD : GoodEventD C fiberBound degreeBound codegreeBound projCodegreeBound)
    (I : Finset (Fin n))
    {ρR ρB xR xB ε ε1 ε2 βdeg qcodeg δsumGap δgapR δgapB : ℝ}
