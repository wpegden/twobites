...
- Dependency order:
  ...
  6. Closed-pair counting layer:
    ...
    - the endpoint cleanups `section4BernoulliMass_le_one`, `section4ActualConditionedEventMass_le_one`, `paper_ri_eqLong_bound_of_outer_smallSum`, and `paper_ri_eqLong_bound_of_outer_le_exp_of_paperRISILossGap_of_le` now remove the small-sum branch from the frontier and package the remaining large / near-one work as a single total-exponent comparison on top of the existing product-level `eq:long` interface, rather than more Bernoulli-mass bookkeeping
  7. Section 4 layer:
