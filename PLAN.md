...
- Dependency order:
  ...
  6. Closed-pair counting layer:
    ...
    - the same three-natural projection-choice route is now lifted through the fixed-set bad-event mass as well: `paperConstructionMass_goodSurvivingIndepSetEventSet_le_sum_if_good_section4ProjectionChoiceMass_of_le` packages the embedding-sum step for one fixed `I` directly at the literal `section4ProjectionChoiceMassSum` level. This means the remaining three-pool collapse can now aim first for a choose-multiplied fixed-set mass bound in terms of the raw naturals `(remaining, u_R, u_B)` before spending any further exponential simplification, which is closer to the paper’s actual finite counting argument.
    - the paper-level extraction step for that exact finite-count route is now packaged too: `exists_triangleFreeWithSmallIndepNum_of_paperChooseMulLe_sum_if_good_section4ProjectionChoiceMass_of_le_lt_explicitGoodMass` feeds the fixed-set `section4ProjectionChoiceMassSum` shell directly into `exists_triangleFreeWithSmallIndepNum_of_paperChooseMulLe_lt_explicitGoodMass_of_massBound`. So once the remaining three-pool combinatorics proves a choose-multiplied bound on that literal fixed-set sum, the final witness theorem follows immediately without another layer of endpoint translation.
    - the raw finite-count route is now also specialized under `paperSection4Bound` itself: the individual opposite-color witness-cap bounds `redProjectionPairCount_outside_section4F_le_partPairCount_LMS_filter_isBlue_of_HPart_subset_section4F2` and `blueProjectionPairCount_outside_section4F_le_partPairCount_LMS_filter_isRed_of_HPart_subset_section4F2`, together with their good-graph-pair wrappers `goodSurvivingGraphPair_redProjectionPairCount_outside_section4F_le_partPairCount_LMS_filter_isBlue_of_paperSection4Bound` and `goodSurvivingGraphPair_blueProjectionPairCount_outside_section4F_le_partPairCount_LMS_filter_isRed_of_paperSection4Bound`, now expose the two witness-cap coordinates separately at the literal LMS-filtered counting level. The new endpoint `goodSurvivingGraphPairProjectionChoiceMassBound_le_section4ProjectionChoiceMassSum_of_paperSection4Bound` packages one graph-pair summand directly in that raw three-natural language, and the lifted shells `paperConstructionMass_goodSurvivingIndepSetEventSet_le_sum_if_good_section4ProjectionChoiceMass_of_paperSection4Bound` and `exists_triangleFreeWithSmallIndepNum_of_paperChooseMulLe_sum_if_good_section4ProjectionChoiceMass_of_paperSection4Bound_lt_explicitGoodMass` now carry that exact LMS-filtered route all the way to the final witness theorem. This means the remaining substantive task is no longer to build wrapper surface, but to prove the choose-multiplied bound for that raw LMS-filtered fixed-set sum from the checked three-pool red/blue cover/cardinality lemmas.
    - that raw `paperSection4Bound` finite-count route can now also be collapsed to a single LMS pair-count budget before any exponential simplification: `section4ProjectionChoiceMassSum_eq_singleBudget_left` shows the finite projection-choice sum depends on the red/blue witness-cap coordinates only through their total, `goodSurvivingGraphPairProjectionChoiceMassBound_le_section4ProjectionChoiceMassSum_of_paperSection4Bound_of_partPairCount_LMS` rewrites the graph-pair endpoint from split filtered LMS caps to the single natural `partPairCount I (L ∪ M ∪ S)`, and the lifted shells `constructionEmbeddingUniformWeight_mul_paperGoodSurvivingGraphPairMass_le_outerMass_mul_sum_if_good_section4ProjectionChoiceMass_of_paperSection4Bound_of_partPairCount_LMS`, `paperConstructionMass_goodSurvivingIndepSetEventSet_le_sum_if_good_section4ProjectionChoiceMass_of_paperSection4Bound_of_partPairCount_LMS`, and `exists_triangleFreeWithSmallIndepNum_of_paperChooseMulLe_sum_if_good_section4ProjectionChoiceMass_of_paperSection4Bound_of_partPairCount_LMS_lt_explicitGoodMass` now carry that one-budget route through fixed embeddings, fixed sets, and final extraction. So the raw finite-count branch no longer needs a color-split LMS witness-cap estimate above graph-pair level; the remaining job is to bound one total LMS pair-count term together with the reveal-budget loss.
    ...
    - the deterministic split closed-error manuscript route now has the matching graph-pair-native open-pair target-gap bridge `goodSurvivingGraphPair_paperSection4OpenPairTargetNat_sub_openError_le_baseOpenPairSet_card_of_uniformPaperErrorBounds_of_paperDeterministicBounds`, built by combining `goodSurvivingGraphPair_uniformPaperErrorBounds_of_paperDeterministicBounds` with the literal split closed-error target-gap theorem. The pointwise shell `goodSurvivingGraphPairProjectionChoiceMassBound_le_exp_of_paperRISILossGap_of_splitClosedErrorBounds_of_paperSection4Bound_of_paperDeterministicBounds` now consumes that lighter bridge instead of reopening the heavier deterministic target-gap wrapper locally, so the remaining split-error route is again focused on manufacturing the actual graph-dependent closed-pair error budgets.
