...
- Dependency order:
  ...
  6. Closed-pair counting layer:
    ...
    - the next concrete subgoal is therefore to bound `sameColorClosedPlusBasePairSet` in terms of the same-color `ClosedPairPlus` contribution and then connect the resulting `T_R` / `T_B` finsets to the exact `U_R` / `U_B` conditional-probability language in `lem:RISI`
  7. Section 4 layer:
