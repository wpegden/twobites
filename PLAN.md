...
- Dependency order:
  ...
  6. Closed-pair counting layer:
    ...
    - the new interface theorem `paper_ri_eqLong_bound_of_outer_le_exp_of_paperRISILossGap` now provides exactly that product-level `eq:long` bridge in the repo’s finite-mass language by multiplying an abstract outer exponential bound with the checked Section 4 `lem:RISI` estimate; the remaining gap is to formalize the actual outer `\mathcal S_{I,\ell_R,\ell_B}` exponential bound and then discharge the last union-bound sum for Theorem `main`
