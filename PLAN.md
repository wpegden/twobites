...
- Dependency order:
  ...
  6. Closed-pair counting layer:
    ...
    - the remaining arithmetic target is to push this concrete large-`n` huge-case specialization farther up the deterministic wrapper chain and determine whether the final paper-facing route can be run entirely through it, or whether the surviving coarse `6 * paperK κ` path still needs a separate paper-note justification
