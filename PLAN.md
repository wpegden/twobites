...
- Dependency order:
  ...
  6. Closed-pair counting layer:
    ...
    - the new comparison wrappers `...of_eventualTwoEpsBranchPieceBounds_of_extraDeficit`, `...of_twoEpsBranchPieceThreshold_of_paramDeficit`, `...of_twoEpsBranchPieceThreshold_of_fourEpsBudget`, `...of_twoEpsBranchPieceThreshold_of_rhoBudget`, `...of_twoEpsBranchPieceThreshold_of_coarseBudget`, and the higher compatibility wrappers `...of_twoEpsBranchPieceThreshold_of_witnessCoeff_of_paramDeficit` / `...of_twoEpsBranchPieceThreshold_of_exactWitnessCoeff_of_paramDeficit` together with `...of_twoEpsBranchPieceThreshold_of_witnessCoeff_of_fourEpsBudget` / `...of_twoEpsBranchPieceThreshold_of_exactWitnessCoeff_of_fourEpsBudget` and `...of_twoEpsBranchPieceThreshold_of_witnessCoeff_of_rhoBudget` / `...of_twoEpsBranchPieceThreshold_of_exactWitnessCoeff_of_rhoBudget` isolate the exact missing higher-level arithmetic: to replace the old coarse `6 * paperK κ` path with the sharpened large-`n` route, it is enough to prove either branchwise extra deficits `δextraR/B` with `2 ε₁ κ ≤ δextraR/B` and `ρ + (1+ε₂)β + 2 ε₁ κ + δextraR/B + δsumGap + δgap ≤ κ`, directly the strengthened canonical budgets `ρ + (1+ε₂)β + 4 ε₁ κ + δsumGap + δgap ≤ κ`, or equivalently the direct `ρ` bounds `ρ ≤ paperHugeWitnessFourEpsRhoBudget β ε₁ ε₂ κ δsumGap δgap`
    - there is now also a manuscript-facing specialization at `β = 1/2`, `κ = 1 + ε`: if a branch projection parameter satisfies `ρ ≤ ((1/2) + ε/4)(1 + ε)` and the explicit lower-order losses obey `ε₂ / 2 + 4 ε₁ (1 + ε) + δsumGap + δgap ≤ ε (1 - ε) / 4`, then that branch automatically lies under `paperHugeWitnessFourEpsRhoBudget`; the new huge-case wrappers specialized to this quarter-projection scale live directly above the threshold-based `ρ` route
    - the asymmetric middle-range route requested by the paper-check is now formalized in manuscript-aligned `ρ`- and `x`-level forms: `Twobites/ParameterBounds.lean` now has `paperHugeWitnessQuarterProjection_of_middleSum_of_blueLeRed`, `paperHugeWitnessQuarterX_of_middleSum_of_blueLeRed`, and `paperHugeWitnessQuarterProjection_of_xMiddleSum_of_blueLeRed`, while `Twobites/IndependentSets.lean` packages the corresponding huge-case theorems as `...of_halfBeta_of_onePlusEps_of_middleRange_of_blueLeRed` and `...of_halfBeta_of_onePlusEps_of_xMiddleRange_of_blueLeRed` together with witness-/exact-witness compatibility versions; this resolves the orientation issue in favor of the relabeled smaller `B`-branch, but it also exposes a real obstruction: the exact remaining coarse dependency is the opposite-branch hypothesis `hblueCrossSmall`, and `Twobites/ParameterBounds.lean` now proves via `not_six_mul_paperK_le_cross_residual` / `..._half_one_add_eps` that such a `6 * paperK κ` residual bound is impossible for the current cross-residual shape, so the next proof step must redesign that branch input rather than derive it from later arithmetic
    - the remaining arithmetic target is therefore no longer a vague “stronger budget”, but those explicit branchwise extra-deficit inequalities, equivalently the concrete `4 ε₁ κ` branch budgets, equivalently the direct upper bounds on `ρ_R` and `ρ_B` given by `paperHugeWitnessFourEpsRhoBudget`; the quarter-projection specialization shows what a successful manuscript-side substitution should look like, and the remaining question is whether the later paper argument really provides that control on both branches or only on the smaller one in the middle regime
