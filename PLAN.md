...
- Dependency order:
  ...
  6. Closed-pair counting layer:
    ...
    - the raw outer counting term from the first lines of `lem:RI` is now formalized twice: first as the exact finite event family `paperRIOuterEventSet` with witness overcount `paperRIOuterWitnessSet`, card bound `paperRIOuterEventSet_card_le_choose_choose_choose`, and finite-mass interfaces `paperRIOuterEventMass_le_outerCombBound` / `paperRIOuterEventMass_le_choose_choose_mul_pow_ratio`, and second as the standalone combinatorial abbreviation `paperRIOuterCombBound` with the exact ratio bound `paperRI_outerChooseRatio_le_pow_ratio` and its lifted form `paperRIOuterCombBound_le_choose_choose_mul_pow_ratio`. The remaining endgame is therefore no longer the finite counting inequality itself, but the missing exponential/asymptotic post-processing that turns this exact outer family into the paper’s `eq:long` exponent before the last union-bound summation
  7. Section 4 layer:
