...
- Dependency order:
  ...
  6. Closed-pair counting layer:
    ...
    - the current concrete specialization chooses `δdegBlue = δcodegBlue = δdegRed = δcodegRed = ε₁ κ`, so `δblue = δred = 2 ε₁ κ`; this is exposed in `paperHugeWitnessBranchwisePieceBranchParamBounds_eventually_le_eps_mul`, `paperHugeWitnessBranchwiseBranchParamBounds_eventually_le_two_eps_mul`, the canonical threshold `paperHugeWitnessTwoEpsBranchPieceThreshold`, and the higher wrappers `...of_eventualTwoEpsBranchPieceBounds` / `...of_twoEpsBranchPieceThreshold_of_extraDeficit`
    - the new comparison wrappers `...of_eventualTwoEpsBranchPieceBounds_of_extraDeficit` and `...of_twoEpsBranchPieceThreshold_of_coarseBudget` isolate the exact missing higher-level arithmetic: to replace the old coarse `6 * paperK κ` path with the sharpened large-`n` route, later parameter lemmas must provide branchwise extra deficits `δextraR/B` with `2 ε₁ κ ≤ δextraR/B` and `ρ + (1+ε₂)β + 2 ε₁ κ + δextraR/B + δsumGap + δgap ≤ κ`
