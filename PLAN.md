...
- Dependency order:
  ...
  6. Closed-pair counting layer:
    ...
    - the current concrete specialization chooses `δdegBlue = δcodegBlue = δdegRed = δcodegRed = ε₁ κ`, so `δblue = δred = 2 ε₁ κ`; this is exposed in `paperHugeWitnessBranchwisePieceBranchParamBounds_eventually_le_eps_mul`, `paperHugeWitnessBranchwiseBranchParamBounds_eventually_le_two_eps_mul`, and the higher wrapper `...of_eventualTwoEpsBranchPieceBounds`, which packages the sharpened exact route under the strengthened budget `ρ + (1+ε₂)β + 4 ε₁ κ + δsumGap + δgap ≤ κ`
    - the new comparison wrapper `...of_eventualTwoEpsBranchPieceBounds_of_extraDeficit` isolates the exact missing higher-level arithmetic: to replace the old coarse `6 * paperK κ` path, later parameter lemmas must provide branchwise extra deficits `δextraR/B` with `2 ε₁ κ ≤ δextraR/B` and `ρ + (1+ε₂)β + 2 ε₁ κ + δextraR/B + δsumGap + δgap ≤ κ`
