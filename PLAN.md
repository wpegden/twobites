...
- Dependency order:
  ...
  6. Closed-pair counting layer:
    ...
    - the remaining arithmetic target is to choose the concrete blue/red branch slacks used later in the paper, specialize the eventual bounds to those targets, and then feed that large-`n` specialization into the surviving exact huge-case wrappers and on to the paper-facing theorems
