...
- Dependency order:
  ...
  6. Closed-pair counting layer:
    ...
    - the new parameter lemma `two_le_paperK_of_two_div_le_of_le_one` and the lifted mixed-case wrappers `paper_risi_hLossGap_of_blueLeft_of_redRight_gapLower_of_paperNearOne_of_two_div_le_loglog` / `paper_risi_hLossGap_of_blueRight_of_redLeft_gapLower_of_paperNearOne_of_two_div_le_loglog` now absorb the separate `2 ≤ paperK (ε (1 - ε) / 8) n` side-condition into the standard large-`n` loglog hypothesis, leaving only the actual near-one image-card and loss inequalities from the paper
  7. Section 4 layer:
