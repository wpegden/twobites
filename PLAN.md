...
- Dependency order:
  ...
  6. Closed-pair counting layer:
    ...
    - the next concrete subgoal is the semantic bridge from this unrevealed base-open-pair finset to the paper’s actual `E_I` / `OpenPairOn I (section4F I ε)` statement, and then packaging the remaining unrevealed pairs in the form needed for the eventual `lem:RISI` proof
  7. Section 4 layer:
