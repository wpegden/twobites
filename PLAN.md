...
- Dependency order:
  ...
  6. Closed-pair counting layer:
    ...
    - the raw outer counting term from the first lines of `lem:RI` is now formalized as the finite combinatorial object `paperRIOuterCombBound`, together with the exact ratio bound `paperRI_outerChooseRatio_le_pow_ratio` and its lifted form `paperRIOuterCombBound_le_choose_choose_mul_pow_ratio`; the remaining endgame is therefore no longer the finite counting inequality itself, but the missing probability-language interface that identifies `P(\mathcal S_{I,\ell_R,\ell_B})` with an instance of that bound and then feeds it into the last union-bound summation
  7. Section 4 layer:
