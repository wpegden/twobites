...
- Dependency order:
  ...
  6. Closed-pair counting layer:
    ...
    - the literal `U_R` / `U_B` layer has also been added as `section4URedPairSet` / `section4UBluePairSet` / `section4UPairSet`, so the remaining Section 4 work is no longer to invent these objects but to prove the semantic bridge from those finsets to the opposite-color witness sets and the exact `lem:RISI` conditional-probability language
  7. Section 4 layer:
