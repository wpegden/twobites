...
- Dependency order:
  ...
  6. Closed-pair counting layer:
    ...
    - the remaining arithmetic target is to prove concrete large-`n` bounds for those two explicit witness pieces (equivalently for `paperHugeWitnessDegreeBranchParam` and `paperHugeWitnessCodegBranchParam`), combine them into the desired blue and red branch slacks, and then feed those exact-piece bounds into the new branchwise exact-piece wrapper and on to the paper-facing wrappers
    - the degree-side `o(k)` term is now explicit in Lean through `paperHugeWitnessDegreeBranchParam_eventually_le`; the unresolved asymptotic input is the projected-codegree piece and the later choice of concrete blue/red branch slacks
