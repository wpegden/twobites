...
- Dependency order:
  ...
  6. Closed-pair counting layer:
    ...
    - the next concrete subgoal is to supply the paper-scale Section 4 arithmetic that specializes that generic hard inclusion, then derive the paper-scale `F_2` cardinality estimate and the unrevealed-open-pair lower bound combining the large/medium/small/huge deterministic lemmas with the reveal-set bookkeeping
  7. Section 4 layer:
