...
- Dependency order:
  ...
  6. Closed-pair counting layer:
    ...
    - the raw outer counting term from the first lines of `lem:RI` is now formalized four ways: as the exact finite event family `paperRIOuterEventSet` with witness overcount `paperRIOuterWitnessSet`, card bound `paperRIOuterEventSet_card_le_choose_choose_choose`, and finite-mass interfaces `paperRIOuterEventMass_le_outerCombBound` / `paperRIOuterEventMass_le_choose_choose_mul_pow_ratio`; as the standalone combinatorial abbreviation `paperRIOuterCombBound` with the exact ratio bound `paperRI_outerChooseRatio_le_pow_ratio` and its lifted form `paperRIOuterCombBound_le_choose_choose_mul_pow_ratio`; as the explicit exponential wrapper `paperRIOuterPowRatioBound` together with `paperRIOuterEventMass_le_powRatioBound`, `paperRIOuterEventMass_le_exp_log_powRatioBound`, and `paperRIOuterEventMass_le_exp_of_log_powRatioBound_le`; and now as the exact logarithmic decomposition `paperRIOuterPowRatioBound_log_eq` plus the separated wrapper `paperRIOuterEventMass_le_exp_of_chooseLogs_le`, which reduce the outer-event asymptotics to bounding two `log choose` terms and one `k * log` ratio term. The remaining endgame is therefore no longer the finite counting inequality itself, but the missing coefficient-level asymptotic post-processing that turns those explicit terms into the paper’s `eq:long` exponent before the last union-bound summation
  7. Section 4 layer:
