...
- Dependency order:
  ...
  6. Closed-pair counting layer:
    ...
    - the new comparison wrappers `...of_eventualTwoEpsBranchPieceBounds_of_extraDeficit`, `...of_twoEpsBranchPieceThreshold_of_paramDeficit`, `...of_twoEpsBranchPieceThreshold_of_coarseBudget`, and the higher compatibility wrappers `...of_twoEpsBranchPieceThreshold_of_witnessCoeff_of_paramDeficit` / `...of_twoEpsBranchPieceThreshold_of_exactWitnessCoeff_of_paramDeficit` isolate the exact missing higher-level arithmetic: to replace the old coarse `6 * paperK κ` path with the sharpened large-`n` route, later parameter lemmas must provide branchwise extra deficits `δextraR/B` with `2 ε₁ κ ≤ δextraR/B` and `ρ + (1+ε₂)β + 2 ε₁ κ + δextraR/B + δsumGap + δgap ≤ κ`
