...
- Dependency order:
  ...
  6. Closed-pair counting layer:
    ...
    - the first finite second-stage wrapper is now checked as well: `section4URedChoiceSet` / `section4UBlueChoiceSet` / `section4UChoiceSet` enumerate candidate conditioned witness outcomes of sizes `u_R,u_B`, `section4UCondPair_images_mem_section4UChoiceSet_of_card_eq_of_indep` places the actual conditioned `U_R,U_B` outcome into that family under independence, and `section4ChoiceMass_le_of_choiceBound_of_le_remaining` plus `openPair_lower_bound_sub_section4_budget_sub_projectionPairCount_sum_sub_oppositeProjectionCounts_le_section4ChoiceMass` convert the deterministic lower bound on `|T \setminus (U_R ∪ U_B)|` into the exact per-`(u_R,u_B)` Bernoulli mass bound; the remaining probabilistic work is therefore the genuine finite conditional-probability wrapper and the subsequent sum over `u_R,u_B`
  7. Section 4 layer:
