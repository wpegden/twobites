...
- Dependency order:
  ...
  6. Closed-pair counting layer:
    ...
    - the raw outer counting term from the first lines of `lem:RI` is now formalized five ways: as the exact finite event family `paperRIOuterEventSet` with witness overcount `paperRIOuterWitnessSet`, card bound `paperRIOuterEventSet_card_le_choose_choose_choose`, and finite-mass interfaces `paperRIOuterEventMass_le_outerCombBound` / `paperRIOuterEventMass_le_choose_choose_mul_pow_ratio`; as the standalone combinatorial abbreviation `paperRIOuterCombBound` with the exact ratio bound `paperRI_outerChooseRatio_le_pow_ratio` and its lifted form `paperRIOuterCombBound_le_choose_choose_mul_pow_ratio`; as the explicit exponential wrapper `paperRIOuterPowRatioBound` together with `paperRIOuterEventMass_le_powRatioBound`, `paperRIOuterEventMass_le_exp_log_powRatioBound`, and `paperRIOuterEventMass_le_exp_of_log_powRatioBound_le`; as the exact logarithmic decomposition `paperRIOuterPowRatioBound_log_eq` plus the separated wrapper `paperRIOuterEventMass_le_exp_of_chooseLogs_le`; and now as the paper-style choose-log layer `choose_cast_le_exp_mul_div_pow`, `paperRI_logChoose_le`, and `paperRIOuterEventMass_le_exp_of_logChooseFormula_le`, which replace the two `log choose` terms by the manuscript’s affine `\ell (1 + \log m - \log \ell)` bounds. The remaining endgame is therefore the asymptotic treatment of the last explicit ratio term `k * (log (l_R l_B) - log (m^2 + 1 - k))` and its combination with those choose bounds into the paper’s `eq:long` exponent before the final union-bound summation
  7. Section 4 layer:
