...
- Dependency order:
  ...
  6. Closed-pair counting layer:
    ...
    - the next concrete subgoal is the later semantic split from `E_I` to `T_R` / `T_B`, namely removing the same-color `ClosedPairPlus` contribution and then packaging the surviving unrevealed pairs in the exact `lem:RISI` probability language
  7. Section 4 layer:
